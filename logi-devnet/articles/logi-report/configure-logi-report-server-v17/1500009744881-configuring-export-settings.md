...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009744881-Configuring-Export-Settings
updated_at: 2021-07-25T07:20:15Z
---

# Configuring Export Settings

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404936712471/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009744901-Configuring-Logs)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404942444695/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009744921-Configuring-the-E-mail-Server)
...
The settings are applied when end users schedule report tasks to [publish report results to e-mail](https://devnet.logianalytics.com/hc/en-us/articles/1500009724282-Examples-of-Scheduling-Reports#Mail).

1. In the Logi Report Server console, point to **Administration** on the system toolbar, and then select **Configuration** > **Export** from the drop-down menu to open the Export page. The E-mail tab is selected by default.

   ![Configure E-mail](https://devnet.logianalytics.com/hc/article_attachments/4404936957847/config_expt_mail.gif)
...
5. Select **Save** and then restart Logi Report Server to make the e-mail settings take effect.
...
Administrators can specify to export the report results either via a fax machine or fax server. The settings will be used when end users schedule report tasks to [publish report results to fax](https://devnet.logianalytics.com/hc/en-us/articles/1500009724282-Examples-of-Scheduling-Reports#Fax).
...
2. In the Logi Report Server console, select **Administration** > **Configuration** > **Export** on the system toolbar, then select the **Fax** tab. The Fax Machine radio button is selected by default.

   ![Configure Fax Machine](https://devnet.logianalytics.com/hc/article_attachments/4404936958103/config_expt_faxmchn.gif)
...
6. From the Flow Control Command drop-down list, select the flow control command according to the modem being used. If not contained in the drop-down list, you can leave this empty and type the command as part of the initial string. The command should be obtained from your modem manual.
7. In the Port text box, type the port number. The port should be obtained from your modem manual.
8. In the Initialization String text box, type the string to initialize the modem. The string should be obtained from your modem manual.
...
11. Select **Save** and then restart Logi Report Server to make the fax settings take effect.

**To export report result to fax via a fax server:**

1. In the Logi Report Server console, select **Administration** > **Configuration** > **Export** on the system toolbar, then select the **Fax** tab.
2. Select the **Fax Server**  radio button.

   ![Configure Fax Server](https://devnet.logianalytics.com/hc/article_attachments/4404936958359/config_expt_faxsrv.gif)
...
6. In the Login ID text box, type the username for the class communicating with fax server.
7. In the Password text box, type the password for the class communicating with fax server.
8. In the Fax Sender text box, specify the user's name that is shown in the fax server manager.
9. In the Special Parameters text box, type the parameters for the fax server.
...
12. Select **Save** and then restart Logi Report Server to make the fax settings take effect.
...
Administrators can customize the precision that will be applied when running reports or exporting reports and dashboards to different formats. However for page reports created in Logi Report Designer, the customized precision can take effect only in report tabs whose Precision Sensitive property is set to true.

1. In the Logi Report Server console, select **Administration** > **Configuration** > **Export** on the system toolbar, and then select the **Layout Precision** tab.

   ![Configure Layout Precision](https://devnet.logianalytics.com/hc/article_attachments/4404942608023/config_expt_prcsn.gif)
2. Select **Customize for each format**.
...
3. Select the **System Default Settings** button, then in the displayed dialog box specify the precision level for each format and select **OK**. The RSD format controls only the Page Report Result format when scheduling to run page report tabs.

   ![System Default Precison Settings](https://devnet.logianalytics.com/hc/article_attachments/4404942608279/config_expt_level.gif)

   Logi Report provides two precision levels: high and low. High precision provides better layout but slower efficiency while low precision brings higher performance but maybe poorer visual effect. If low precision can make give reports well looks, it is reasonable to apply low precision for at the same time faster performance is guaranteed. By default Logi Report applies high precision for formats such as PDF, RTF, Excel, Fax and PostScript, thus the report result layouts of these formats are different from the other formats such as HTML, Logi Report Result, XML, and Text.
4. In the Layout Precision tab, select the check box for the required formats to apply the defined precision level. For formats that are not selected, Logi Report will decide their precision.
5. Select **Save** and then restart Logi Report Server to make the settings take effect.

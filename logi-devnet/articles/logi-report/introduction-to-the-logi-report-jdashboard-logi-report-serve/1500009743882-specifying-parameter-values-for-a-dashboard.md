...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009743882-Specifying-Parameter-Values-for-a-Dashboard
...
If you want to change the parameter values, take the following steps after you open a dashboard in JDashboard:

1. Select the **Enter Parameter Values** button ![Enter Parameter Values button](https://devnet.logianalytics.com/hc/article_attachments/4404880189463/btn_dshbrd_prm.gif) on the toolbar. JDashboard displays the [Enter Parameter Values](https://devnet.logianalytics.com/hc/en-us/articles/1500009745482-Enter-Parameter-Values-Dialog-Box-Properties) dialog box.

   ![Enter Parameter Values dialog](https://devnet.logianalytics.com/hc/article_attachments/4404880454039/enterprmvlu.gif)
2. Edit the parameter values according to your requirement.
   ...
   * In the parameter value combo box, select the down arrow ![Drop Down button](https://devnet.logianalytics.com/hc/article_attachments/4404885319191/btn_dshbrd_more1.gif) and then select a value from the drop-down list. To ensure performance, Server lists at most 500 values by default.
   ...
   * Select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4404885317143/btn_clndr.gif) to [specify a date and time value](https://devnet.logianalytics.com/hc/en-us/articles/1500009778361-Specifying-Parameter-Values#DateTime).
...
**Tip:** If you want to make the **Enter Parameter Values** dialog box display when running dashboards that contain parameters, you can select the JDashboard profile option [Show Enter Parameter Values Dialog](https://devnet.logianalytics.com/hc/en-us/articles/1500009771841-Customizing-JDashboard-Profile#Show).
...
1. Select in the value combo box of the parameter. Server displays the [Enter Values](https://devnet.logianalytics.com/hc/en-us/articles/1500009773341-Enter-Values-Dialog-Box-Properties) dialog box.

   ![Enter Values dialog](https://devnet.logianalytics.com/hc/article_attachments/4404880453527/entervlu.gif)
2. If the parameter's **Enable the "All Values" Option** property is true, the **All Values** check box is available. JDashboard selects it by default. It means to apply all values to the parameter. When you inserted the parameter as a field into a report and selected **All Values**, the field shows the string **All**. If you do not want to apply all values to the parameter, clear the **All Values** check box, then specify the desired values as shown in the following steps.
3. Select the required values in the **Available Values** box and select the button ![Add Values](https://devnet.logianalytics.com/hc/article_attachments/4404880147479/btn_add2.gif) to add them to the **Selected Values** box; to add all the listed values, select the button ![Add All Values](https://devnet.logianalytics.com/hc/article_attachments/4404880147735/btn_addall1.gif). To ensure performance, by default, JDashboard displays at most 500 values in the **Available Values** box.

   For values you do not want, select them in the **Selected Values** box and then select the button ![Remove Values](https://devnet.logianalytics.com/hc/article_attachments/4404880147991/btn_rmv1.gif) to remove them; to remove all values you added, select the button ![Remove All Values](https://devnet.logianalytics.com/hc/article_attachments/4404880148503/btn_rmvall1.gif).
4. When the parameter's **Allow Type-in of Value** property is true, the **Enter Values** text box is available. You can type a value manually and select the button ![Add Values](https://devnet.logianalytics.com/hc/article_attachments/4404880147479/btn_add2.gif) next to add it to the **Selected Values** box. When the parameter is bound with a column, but the display column is different from the bound column, make sure the value you type is that of the bound column. After you type text, JDashboard filters the values in the **Available Values** box and display those containing the text. To return to the original list, clear the input text. If the parameter is of the Date, DateTime, or Time type, you can also select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4404885317143/btn_clndr.gif) to [specify a date and time value](https://devnet.logianalytics.com/hc/en-us/articles/1500009778361-Specifying-Parameter-Values#DateTime).
...
![Note icon](https://devnet.logianalytics.com/hc/article_attachments/4404885305111/noteicon.jpg)When using the Save Default Parameter Values feature, you need to be aware of the following:
...
* JDashboard remembers the last-time saved result of a dashboard and displays the exact result the next time you open the dashboard. In the case you set USA as the default parameter value and then filter the result with Canada and save the dashboard, the next time the dashboard will run with Canada, though Server still keeps the saved default parameter value as USA. If you want to run the dashboard with USA, select the option **Use User Defined Default Parameter Values** in the My Profile > Customize Profile > JDashboard > Properties tab.
...
To switch on the function of saving parameter values, you need to configure the server profile: select **Yes** to the option [Enable Saving Parameter Values](https://devnet.logianalytics.com/hc/en-us/articles/1500009743782-Configuring-Server-Profile#SaveParam), then select in which way to save the values: Manually or Automatically, and specify a maximum number to limit the saved value groups via the [Maximum Number of Auto Complete Parameters List](https://devnet.logianalytics.com/hc/en-us/articles/1500009743782-Configuring-Server-Profile#MaxNo) option for each user-dashboard pair.

**Manually saving parameter values**

When you have chosen to manually save parameter values, Server displays the **Use Saved Values** button ![Use Saved Values button](https://devnet.logianalytics.com/hc/article_attachments/4404880454295/btn_dshbrd_ussvvlu.gif) in the Enter Parameter Values dialog box. By selecting this button, you will get the following:
...
* ![Save button](https://devnet.logianalytics.com/hc/article_attachments/4404885307671/btn_save.gif)**Save Values**  
   Select the button to save the current displayed parameter values as a value group for reuse afterwards. To do this, type a name for the group in the text box which shows **Save current values into list** and then select the button **![Save button](https://devnet.logianalytics.com/hc/article_attachments/4404885307671/btn_save.gif)**. Server adds the saved value group into the value list.
...
You can access the configuration panel of a library component to specify its parameter values. To do this, select the button ![Options icon](https://devnet.logianalytics.com/hc/article_attachments/4404880135063/btn_dshbrd_more.gif) on the [component title bar](https://devnet.logianalytics.com/hc/en-us/articles/1500009771801-General-Operations-in-JDashboard#TitleBar) and select **Edit Setting** from the drop-down menu. Server displays the configuration panel. Specify the values in the panel and select **OK** to apply them to the library component. You can select **Cancel** to close the configuration panel.

You can select **Show the panel by default** in the panel to show the configuration panel before rendering the library component each time the dashboard runs or refreshes. For example, if a library component uses parameters, and you would like to specify parameter values before loading the component rather than first loading the component and then changing the parameter values to run the component the second time to get the desired result, you can select this option to display the panel before rendering the component by default.
...
1. Select the **Options** button ![Options button](https://devnet.logianalytics.com/hc/article_attachments/4404880435735/btn_dshbrd_optn.gif) on the toolbar and then select **Share Parameter** from the option list. JDashboard displays the [Share Parameters Setting](https://devnet.logianalytics.com/hc/en-us/articles/1500009773601-Share-Parameters-Setting-Dialog-Box-Properties) dialog box.

   ![Share Parameters Setting dialog](https://devnet.logianalytics.com/hc/article_attachments/4404885521687/shareprm.gif)

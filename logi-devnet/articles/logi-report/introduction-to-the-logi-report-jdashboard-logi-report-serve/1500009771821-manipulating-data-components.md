...
For more information, see [Going Through Web Report Data](https://devnet.logianalytics.com/hc/en-us/articles/1500009778781-Going-Through-Web-Report-Data).
...
![Filter Condition Created by Going Action](https://devnet.logianalytics.com/hc/article_attachments/4404885602839/dshbrd_mnplt_go.gif)
...
![Data Format Submenu](https://devnet.logianalytics.com/hc/article_attachments/4404885603223/dshbrd_mnplt_format1.gif)
...
* **Filtering data in a table**  
  ...
  + **Using shortcut menu**   
    ...
    ![Filter Submenu](https://devnet.logianalytics.com/hc/article_attachments/4404880560151/dshbrd_mnplt_filter.gif)
    ...
    - **Select Values**   
      Brings up the [Select Values](https://devnet.logianalytics.com/hc/en-us/articles/1500009773641-Select-Values-Dialog-Box-Properties) dialog box to select the values to filter the field with.
  + **Using the filter button on table column headers**  
     You can use the filter button on any table column header to filter values in the column if the feature is enabled in the [server profile](https://devnet.logianalytics.com/hc/en-us/articles/1500009743782-Configuring-Server-Profile#ColumnHeader). By default, the feature is disabled so you need to configure the profile before you can use it.
    ...
    3. Put the mouse pointer on any column header and you can find the filter button ![Filter button](https://devnet.logianalytics.com/hc/article_attachments/4404880192279/btn_lblfltr.gif). Select the button and a filter list which contains the same items as the [Filter submenu](#Menu) appears.
...
* **Sorting data in a group table**  
  ...
  + **Using shortcut menu**  
    ...
    ![Sort Submenu](https://devnet.logianalytics.com/hc/article_attachments/4404885603607/dshbrd_mnplt_sort.gif)
  ...
  + **Using the sort button on table column headers**  
     You can use the sort button on any table column header to sort values in the column, because Server enables the feature in the [server profile](https://devnet.logianalytics.com/hc/en-us/articles/1500009743782-Configuring-Server-Profile#ColumnHeader) by default.
    1. In the Common tab of the My Profile > Customize Profile page or Administration > Server Profile > Customize Profile page, make sure the option Sort on Column Headers is selected, then select **Show Sort/Filter Status on Column Headers** if you want the sort button to be displayed with the current sort status on the table column headers after the columns are applied with sort orders. Select **OK** to save the profile setting.
    2. Run a dashboard containing a table.
    3. Put the mouse pointer on any column header and you can find the sort button ![Sort button](https://devnet.logianalytics.com/hc/article_attachments/4404880150935/btn_lblsort.gif).
...
* **Sorting data in a summary table**  
   You can use the [same methods on group tables](#Group) to sort the group and summary columns in summary tables. However, for summary tables, you can define whether to use major sort or minor sort via the [Use Major Sort as Default](https://devnet.logianalytics.com/hc/en-us/articles/1500009743782-Configuring-Server-Profile#Sort) option in the server profile. By default, major sort is applied which is to sort data in a summary table by the last sort condition only, that is to say the later sort condition always replaces the former one. When minor sort is used, sort of a lower group is always based on the sort result of all its higher-level groups.
...
* **Swapping chart groups**  
  ...
  ![Chart Shortcut Menu](https://devnet.logianalytics.com/hc/article_attachments/4404885603863/dshbrd_mnplt_chart.gif)
...
* **Configuring real time chart settings**  
  ...
  1. Right-click the chart and select **Real-time Settings** from the shortcut menu. Server displays the [Real-time Settings](https://devnet.logianalytics.com/hc/en-us/articles/1500009745622-Real-time-Settings-Dialog-Box-Properties) dialog box.

     ![Real-time Settings dialog](https://devnet.logianalytics.com/hc/article_attachments/4404885521943/rltmset.gif)
  ...
  4. Select the **Incremental Fetch** button to add the fields you want to use as the unique key of the real time chart in the [Unique Key](https://devnet.logianalytics.com/hc/en-us/articles/1500009773681-Unique-Key-Dialog-Box-Properties) dialog box.
...
* **Switching crosstab fields**  
  ...
  ![Crosstab Shortcut Menu](https://devnet.logianalytics.com/hc/article_attachments/4404885604119/dshbrd_mnplt_crstb.gif)

---
title: "Logi JReport Server Cluster Main Features"
id: 1500009686502
section: "Server Cluster Logi JReport Server v16"
category: "Logi Report"
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009686502-Logi-JReport-Server-Cluster-Main-Features
updated_at: 2021-11-03T06:58:29Z
---

# Logi JReport Server Cluster Main Features

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4412131374359/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009712281-Logi-JReport-Server-Cluster)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4412139481879/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009712321-Setting-Up-and-Starting-a-Logi-Report-Server-Cluster)
...
* **Weighted Min-load (loadbalance.type=2)**  
  ...
  Performance weight is a positive floating point number that you set to each server in a cluster on any clustered server. Use the Administration > Configuration > Cluster > Weight page in the Logi JReport Server console and measure the performance of a typical report on each node of the cluster. The higher performance weight you set to a clustered server, the higher chance it may get selected by the server that holds the active scheduler during load balancing. See [Configuring Performance Weight](https://devnet.logianalytics.com/hc/en-us/articles/1500009712301-Managing-a-Logi-Report-Server-Cluster#Weight) for how to set performance weight and how this algorithm works.
...
You can check the status of the clustered servers on Logi JReport Server Monitor and notice the failure of any member server. If a member server is down, Logi JReport Server Cluster will remove it from the active clustered server list.

**Member server failure**

* **Effect on load balancing**When Logi JReport Server Cluster detects a failed member server, it will remove the member server from the active server list and will not schedule reporting tasks to that server any more. Load balancing will proceed on the remaining active servers.
* **Effect on incomplete tasks**When Logi JReport Server Cluster detects a failed clustered server, it will check the shared table for the list of incomplete tasks and will then reassign all incomplete tasks to other active servers using the load balancer.
...
If you have enabled the notifying of server down feature, when a member server crashes or is disconnected with the cluster, Logi JReport Server Cluster will send a notification e-mail to a specified address.

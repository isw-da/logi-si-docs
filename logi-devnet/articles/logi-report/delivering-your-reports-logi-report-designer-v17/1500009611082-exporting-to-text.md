...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009611082-Exporting-to-Text
updated_at: 2021-07-24T16:03:57Z
---

# Exporting to Text

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404911578903/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009634081-Exporting-to-Excel) [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404911579543/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009634141-Exporting-to-RTF)

# Exporting to Text

Logi Report supports exporting to both a regular Text file and a CSV file.

This topic includes the following sections:

* [Exporting the Report Result to a Text File](#Text)
* [Exporting the Report Result to a CSV File](#CSV)
* [Improving the Performance of Running a Report to Delimited Format on Logi Report Server](#Performance)
...
2. Select **File** > **Export**  >  **To Text**. The [Export to Text](https://devnet.logianalytics.com/hc/en-us/articles/1500009608182-Export-to-Text-Dialog) dialog appears.

   ![Export to Text dialog](https://devnet.logianalytics.com/hc/article_attachments/4404911669271/expt2txt.gif)
3. Select the **Browse** button to specify the destination directory and file name of the exported Text file. You can also type the location and file name manually in the Export to text box (make sure that the folder you specify does exist, otherwise an error message will be produced).
...
6. Specify whether to use user defined density to export the report result. If yes, specify the horizontal density and vertical density respectively ([go here](#Density) for some notes about setting the density):
   * **Horizontal Density**  
...
8. By default, all headers and footers in the report will be exported, including Report Header/Footer, Page Header/Footer and Group Header/Footer. If you only want to export data in the Detail panel, clear **Header and Footer**.
...
5. Select **Delimited Format**  checkbox, and then select **CSV Format**  from the Format drop-down list.
6. To enable records to be aligned well in the exported file, you are recommended to select **Use Quote Mark**. Otherwise, the commas in data values such as $78,321.00 will conflict with the delimiter (comma), and may potentially cause the data to be separated incorrectly when the exported file is further processed by other applications. The following shows an example:
...
7. Select **Repeat Last Column Value If Null** so that when a cell in the exported CSV result has no value, value of the previous cell in the same column will be used.
8. You can specify the other settings. Here we keep them to the defaults.
...
* Whether a report is exported to a general CSV file or a columned CSV file,
  + The data of each field should be in one column. If there is no data in a column, empty strings (" ") will be placed there automatically. In which case, the empty strings cannot be compressed. Only if there is no data in the entire line, can a row be compressed when you select the Compress option.
  + In the Export to Text dialog, if you select Custom Delimited, you should export the report to a .txt file instead of a .csv file. Then, open the exported file with Excel, and the Text Import Wizard will be displayed. In this wizard, select the **Delimited** radio button, select **Next**, select **Other** on the Delimiters panel and key in the delimiter you specify in the Export to Text dialog, and then select **Finish** to view the result.
...
* When you specify to export a report to a columned CSV file,
  + Objects in the exported file will be located according to their Row Index and Column Index property values. When setting values for these two properties, pay attention to the following:
    ...
    - If either the Row Index value or Column Index value of an object is less than 1, Logi Report will ignore this object and all its children.
...
## Improving the Performance of Running a Report to Delimited Format on Logi Report Server

When exporting a page report to Text, if you want to improve the performance of running the page report to Delimited Format (such as the CSV format) on Logi Report Server, you are first required to set certain settings in Logi Report Designer. After which you can then publish the report to Logi Report Server for running to Delimited Format. See the example below:

1. Start Logi Report Designer and open the page report tab you want to run to Delimited Format on Logi Report Server.
...
5. Publish the page report containing the report tab to Logi Report Server. When you run or schedule the report, after you choose the report tab and then choose To Text format, select the **Delimited Format** box, and then select **Submit** to generate a Delimited Format file.

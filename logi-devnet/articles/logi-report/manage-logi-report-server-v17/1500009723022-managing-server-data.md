...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009723022-Managing-Server-Data
updated_at: 2021-07-25T07:18:59Z
---

# Managing Server Data

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404936712471/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009750161-Managing-Tasks)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404942444695/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009750001-Managing-Cached-Report-Data)

# Managing Server Data

During its running process, Logi Report Server keeps track of server information and stores it to its own database for the purpose of running various managing and monitoring tasks, such as managing the resources on the server, monitoring the task running status, and collecting server running statistics.
...
[Configuring the Server Database](https://devnet.logianalytics.com/hc/en-us/articles/1500009744781-Configuring-the-Server-Database) for details about how to configure the databases.

This topic shows managing server data as follows. You must be an admin user in order to access the Administration menu in the server console to perform the data management tasks.
...
* **Backing up the server data via server console**
  ...
  2. Select the **Backup** tab.
     ...
     ![Backup tab](https://devnet.logianalytics.com/hc/article_attachments/4404936801175/mng_data_back.gif)
  ...
  5. If you are backing up the system database, the With External Data check box is available. Select it if you want the external data bound with the database which are the physical resources in the server resource tree to be archived at the same time.
...
* **Backing up the server data by command line**
  ...
  A tool is provided to backup the server data. It is [DBMaintain.bat/DBMaintain.sh](https://devnet.logianalytics.com/hc/en-us/articles/1500009749961-Running-as-a-Standalone-Server#DBMaintain) in <`install_root>\bin`. It has two parameters:

  + -Bsystemtables/-Brealmtables/-Bprofiling: Backup the data with the server data in a Logi Report Server Cluster.
...
Along with the running of the server, the sizes of the result version table (DB: realm) and TaskContext table (DB: profiling) grow larger. Logi Report Server allows you to archive data to a single backup file and then automatically delete it from the database. You can then retrieve the data from the backup file at a later date. However you can only restore server data from an archive file that is archived using the same server version.

Only administrators can archive and restore server data in the realm and profiling databases.

**To archive the server data:**

1. In the Logi Report Server console, point to **Administration** on the system toolbar, and then select **Configuration** > **Server DB** > **Realm DB**/**Profiling DB** from the drop-down menu.
2. In the Realm DB/Profiling DB page, switch to the **Archive** tab.

   ![Archive tab](https://devnet.logianalytics.com/hc/article_attachments/4404942507159/mng_data_archive.gif)
...
5. ![This image notes any new content for version 17 of the product. For more information please see the Release Notes or What's New topics.](https://devnet.logianalytics.com/hc/article_attachments/4404942452887/__newv17.jpg "New for version 17.")Specify the range of data you want to archive. The range is defined using date time.

   For Realm DB, in the Move to Archive Data After and/or Move to Archive Data Before text boxes, specify the date time accordingly (at least one date time should be set). Logi Report Server will then archive the data which is generated between the specified time range or after/before the specified date time.

   For Profiling DB, specify the date time in the Move to Archive Data Before text box to let Logi Report Server archive the data that is generated before the date time.
6. Type or select the **Browse** button to specify the file path and name for the archive file.

   If you don't want to back up the archived server data, leave this field blank. The server data will then be removed from the database.
7. Select **Archive**. Server data within the specified time period will be archived and saved to the archive file using the provided name and the same data will be deleted from the database.
...
2. In the Realm DB/Profiling DB page, switch to the **Summary** tab.
   ...
   ![Summary tab](https://devnet.logianalytics.com/hc/article_attachments/4404936801687/mng_data_view.gif)
3. Select a realm from the Select Realm drop-down list at the top right corner.
...
1. In the Logi Report Server console, point to **Administration** on the system toolbar, and then select **Configuration** > **Server DB** > **Realm DB** from the drop-down menu. Report Server displays the Realm DB page.
2. Select the **Realm DB Check** tab.

   ![Realm DB Check tab](https://devnet.logianalytics.com/hc/article_attachments/4404942507671/mng_data_clear.gif)
3. Select a realm from the Select Realm drop-down list at the top right corner.
...
In abnormal circumstances, server data may not be saved correctly or completely in the database. Logi Report Server allows you to check the integrity of server data.
...
* Launch Logi Report Server on the command line with the -cleanup parameter. For example:

  `C:\LogiReport\Server\bin>JRServer -cleanup`
* Open the batch file JRServer.bat stored in `<install_root>\bin`, add the parameter -Dcheck.realmtables=true as follows, then start Logi Report Server with the edited batch file.

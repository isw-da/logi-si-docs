...
1. Right-click the chart and select **Chart Wizard** from the shortcut menu to display the [Chart Wizard](https://devnet.logianalytics.com/hc/en-us/articles/1500010064921-Chart-Wizard-Dialog).
...
4. In the Display screen, specify the fields to be displayed on the chart.
   Use the button ![Replace button](https://devnet.logianalytics.com/hc/article_attachments/4404901173527/btn_replace.gif) to replace a current field. For details about how to define a chart, see [Inserting Charts in a Report](https://devnet.logianalytics.com/hc/en-us/articles/1500010063601-Inserting-Charts-in-a-Report).
...
1. When creating or editing a chart with wizard, select the **Special Group** button in the Display screen. The [User Defined Group](https://devnet.logianalytics.com/hc/en-us/articles/1500010068301-User-Defined-Group-Dialog) dialog appears.
...
3. Check **Keep values outside of the range in special group** checkbox if you want to put the values that are not included in the specified criteria in a new special group, and then provide a name for the special group in the Special Group Name text box.
...
1. When creating or editing a chart with wizard, select the **Special Function** button in the Display screen. The Special Function dialog appears.
   ...
   ![Special Function dialog](https://devnet.logianalytics.com/hc/article_attachments/4404901179543/spclfctn.gif)
2. Select the [special function](https://devnet.logianalytics.com/hc/en-us/articles/1500010063921-Grouping-the-Data-in-Tables#FunctionList) for the category/series/group field from the Function drop-down list, then select **OK**.
...
1. When creating or editing a chart with wizard, select the **Order/Select N**  button in the Display screen.The [Category Options](https://devnet.logianalytics.com/hc/en-us/articles/1500010065241-Category-Options-Dialog)/[Series Options](https://devnet.logianalytics.com/hc/en-us/articles/1500010032382-Series-Options-Dialog)/[Group Options](https://devnet.logianalytics.com/hc/en-us/articles/1500010031322-Group-Options-Dialog) dialog appears.
...
7. If necessary, check **Skip First** and input a number M in the text box to the right, then the first M category/series/group values will be skipped, and the Select N condition will begin with M+1. The skipped values will be merged into the Remaining Categories group.
...
4. Save the report and select the **View** tab to preview the chart. The chart appears as follows:
   ...
   ![Chart Result Based on BV](https://devnet.logianalytics.com/hc/article_attachments/4404901335319/cmpnt_cht_mdfy_slctn1.gif)
...
7. In the Category Options dialog, set the Select condition to Top 6, based on Total Sales descending, then check the **Remaining Categories In** checkbox and input **Other** into the text box so as to put the categories that do not meet the condition into the Other group. Select **OK** to accept the settings and leave the dialog.

   ![Set Category Options](https://devnet.logianalytics.com/hc/article_attachments/4404909285015/cmpnt_cht_mdfy_slctn2.gif)
8. Select **Finish** in the Chart Wizard to finish editing the chart.
9. Preview the chart again. It now shows as follows:

   ![Chart Result](https://devnet.logianalytics.com/hc/article_attachments/4404901335831/cmpnt_cht_mdfy_slctn3.gif)
10. Go back to the design mode and repeat step 5 to 7.
11. In the Category Options dialog, check the **Skip First** checkbox and input **2** in the text box, then select **OK**.
12. Select **Finish** in the Chart Wizard.
13. Preview the chart and the result changes to below:

    ![Chart Result for Skipping the First 2 Categories](https://devnet.logianalytics.com/hc/article_attachments/4404901336087/cmpnt_cht_mdfy_slctn4.gif)

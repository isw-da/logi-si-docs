...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500010028842-Formatting-the-Gauge-in-a-Gauge-Chart
updated_at: 2021-07-24T10:39:28Z
---

# Formatting the Gauge in a Gauge Chart

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404901108631/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500010063561-Formatting-the-Rectangles-and-Rectangle-Titles-in-a-Heat-Map) [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404901037591/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500010028962-Formatting-the-Surface-in-a-Surface-Chart)
...
3. In the Axis tab, specify properties of the chart axis.
   1. In the Axis sub tab, set properties of the axis.
      ...
      ![Format Solid Gauge dialog - Axis - Axis1](https://devnet.logianalytics.com/hc/article_attachments/4404909197463/fmtsldgauge_axis_axis1.gif)

      Check **Show Axis** to display axis in the solid gauge. It is meaningful to check the option so that the other properties related with the axis can work.

      In the Type box, specify the position of the axis, which can be inside, outside or in the center of the arcs.

      In the Option box, specify the minimum and maximum values to be displayed on the axis or [use a formula to control the values](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties).

      In the Line box, specify the color and transparency of the axis or check **Use Range Color** to [use the color defined for the ranges](#SolidRangeColor) as the axis color. Specify the style and thickness of the axis.
   ...
   2. In the Tick Mark sub tab, specify properties of the tick marks on the axis.
      ...
      ![Format Solid Gauge dialog - Axis - Tick Mark](https://devnet.logianalytics.com/hc/article_attachments/4404909197719/fmtsldgauge_axis_tkmk.gif)

      To display tick marks on the axis, check how to show it in the Type box: inside, outside or in the center of the axis.

      In the Major Tick Mark Line and Minor Tick Mark Line boxes, specify the distance between two adjacent tick marks on the axis, how many tick marks to display on the axis, and the color, style, transparency, thickness and length of the tick marks respectively. If you want to use the same line setting as that of the axis for the tick mark lines, check **Correlate with Axis**.
   3. In the Label sub tab, specify properties for the data labels on the axis.

      ![Format Solid Gauge dialog - Axis - Label](https://devnet.logianalytics.com/hc/article_attachments/4404901222679/fmtsldgauge_axis_lbl.gif)
      ...
      * **Normal**  
        ...
        check **Auto** if you want all major tick mark labels to be shown on the axis or **Fixed** to show only the specified number of major tick mark labels.
      * **Range Value**  
      ...
      * **Min and Max Values**  
      ...
      In the Gap box, specify the distance between the data labels and the axis in pixels and whether to adjust the labels automatically to make them placed best. When Best Effect is checked, some labels will be hidden when they are overlapped.
...
4. In the Pointer tab, specify properties for the pointers of the arcs in the solid gauge.
   ...
   * To specify color for the pointers separately, leave Use Range Color uncheck. First make a choice for the Self Settings checkbox: when Self Settings is unchecked, the color pattern specified here will be synchronized to the [Pattern List property](https://devnet.logianalytics.com/hc/en-us/articles/1500010069721-Chart-Properties#Pattern) on the chart object in the Report Inspector, which can also be applied by data markers of other subtypes if the chart is a combo chart; when Self Settings is checked, it indicates the color pattern is private to the current data markers themselves (the pointers in this case), which can be remembered and applied to the data markers of a new type automatically if later you change the type of the chart. Then specify the color and the transparency of the color schema to fill the pointers in the same data series as the arc range you have selected on to open the Format Solid Gauge dialog, or select the **Color List** button to specify the color pattern for pointers in the same data series respectively in the [Color List](https://devnet.logianalytics.com/hc/en-us/articles/1500010030022-Color-List-Dialog) dialog. The colors defined for the pointers will also apply to the corresponding [ranges in the arcs](#ArcColor).
   ...
   In the Pointer Border box, check **Show Pointer Border** if you want to show the border of the pointers, then set properties of the border including line style, thickness, color, and transparency for color.
5. In the Target tab, specify properties of the target in the solid gauge.

   ![Format Solid Gauge dialog - Target](https://devnet.logianalytics.com/hc/article_attachments/4404909197975/fmtsldgauge_tgt.gif)

   Check **Use Target Value** if you want to use target value for the solid gauge, and specify the target value as required. You can input the value in the text box directly or [use a formula to control the value](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties).
...
6. In the Frame tab, specify properties for the frame of the solid gauge.
   ...
   ![Format Solid Gauge dialog - Frame](https://devnet.logianalytics.com/hc/article_attachments/4404909198231/fmtsldgauge_frm.gif)
...
7. In the Range Color tab, select in the corresponding cells to set the minimum and maximum values, colors from the [color palette](https://devnet.logianalytics.com/hc/en-us/articles/1500010035022-Menu-Tabs#Palette), and names for the three predefined value ranges in each arc to fill the arcs with colors. To add a new color range, select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404901125271/btn_add.gif), and if a color range is not required, select ![Remove button](https://devnet.logianalytics.com/hc/article_attachments/4404909120791/btn_rmv.gif) to remove it. Then in the Others box, specify the name and color for the values that do not fall into any of the ranges you define.

   ![Format Solid Gauge dialog - Range Color](https://devnet.logianalytics.com/hc/article_attachments/4404909198487/fmtsldgauge_rgclr.gif)
8. In the Hint tab, specify whether to
   include the category and series values and whether to scale big and small numbers in the gauge hint. A hint displays the value a range/pointer of an arc in the solid gauge represents when the mouse pointer points at the range/pointer in Logi JReport Designer view mode, in HTML result, or at server runtime. To make the hint shown, you need to make sure the [Show Tips](https://devnet.logianalytics.com/hc/en-us/articles/1500010033982-Chart-Paper-Properties#ShowTips) property in the Report Inspector is set to true.

   ![Format Solid Gauge dialog - Hint](https://devnet.logianalytics.com/hc/article_attachments/4404909198743/fmtsldgauge_hint.gif)
9. For a solid gauge chart in a library component, you can define web behaviors on it in the Behaviors tab.

   ![Format Solid Gauge dialog - Behaviors](https://devnet.logianalytics.com/hc/article_attachments/4404901223575/fmtsldgauge_bhvr.gif)
...
1. Right-click any pointer in a dial of the dial gauge and select **Format Dial Gauge** on the shortcut menu, or double-click any pointer in the dial gauge. The [Format Dial Gauge](https://devnet.logianalytics.com/hc/en-us/articles/1500010066401-Format-Dial-Gauge-Dialog) dialog appears.

   ![Format Dial Gauge dialog - Circular Graph](https://devnet.logianalytics.com/hc/article_attachments/4404909210391/fmtdlgauge_crculr.gif)
2. In the Circular Graph tab, specify properties for the dials in the dial gauge.
   ...
   Check **Show Range Name** if you want to show the names of the ranges you define, then specify the font formats and effects of the range names in the Font and Effects boxes.
3. Go to the Axis tab to specify properties of the chart axis.
   1. In the Axis sub tab, set properties of the axis.

      ![Format Dial Gauge dialog - Axis - Axis1](https://devnet.logianalytics.com/hc/article_attachments/4404901238295/fmtdlgauge_axis_axis1.gif)

      Check **Show Axis** to display the axis. In the Type box, specify the position of the axis, which can be inside, outside or in the center of the dials.

      In the Option box, specify the minimum and maximum values to be displayed on the axis, or [use a formula to control the values](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties). The values will be equally divided into three ranges, each of which will be filled with the color specified in the Color Range tab automatically.

      In the Line box, specify the color and transparency of the axis or check **Use Range Color** to [use the color defined for the ranges](#DialRangeColor) as the axis color. Specify the style and thickness of the axis.

      In the Gap box, specify the gap between the axis and the dials if the axis is displayed inside or outside of the arcs.
   2. In the Tick Mark sub tab, specify properties of the tick marks on the axis.

      ![Format Dial Gauge dialog - Axis - Tick Mark](https://devnet.logianalytics.com/hc/article_attachments/4404901241751/fmtdlgauge_axis_tkmk.gif)
   ...
   3. In the Label sub tab, specify properties for the data labels on the axis.
      ...
      ![Format Dial Gauge dialog - Axis - Label](https://devnet.logianalytics.com/hc/article_attachments/4404901242135/fmtdlgauge_axis_lbl.gif)
      ...
      * **None**  
         If selected, the data labels will not be shown on the axis.
      * **Normal**  
         If selected, the data labels will be shown as you specify: in the Label Every N Major Tick Marks combo box, set the frequency at which the major tick marks will be labeled; check **Auto** if you want all major tick mark labels to be shown on the axis or **Fixed** to show only the specified number of major tick mark labels.
      * **Range Value**  
   ...
   4. In the Format sub tab, specify the data format of the data labels on the axis.
      ...
      ![Format Dial Gauge dialog - Axis - Format](https://devnet.logianalytics.com/hc/article_attachments/4404901242391/fmtdlgauge_axis_fmt.gif)

      Select a category from the Category box, then select a format from the Format box and select **Add** to add it to the Stack box. If the formats listed in the Format box cannot meet your requirement, define the format in the Properties text box and select **Add** to add it as the format of the selected category. For the Number category, you can specify whether to scale big and small numbers by setting the [Auto Scale in Number](https://devnet.logianalytics.com/hc/en-us/articles/1500010066401-Format-Dial-Gauge-Dialog#AutoScale) option. You can add more than one format, but for each category only one format can be added. In the event that a format is not necessary, select it in the Stack box select **Remove** to clear it.
4. In the Pointer tab, specify properties for the dial pointers.

   ![Format Dial Gauge dialog - Pointer](https://devnet.logianalytics.com/hc/article_attachments/4404909210647/fmtdlgauge_pnt.gif)

   In the Pointer Style box,

   * To use arrow as the pointer style, check **Arrow**, then specify the style of the value pointer, and the width and height of the arrow. If required, select **Style List** to specify the style for pointers in the same data series respectively in the [Style List](https://devnet.logianalytics.com/hc/en-us/articles/1500010068061-Style-List-Dialog) dialog.
   * To use mark as the pointer style, check **Mark**, then specify the style of the value pointer, the width and height of the mark, the position relationship of the marks relative to the dial, and the distance between the pointer and the dial in pixels. If required, select **Style List** to specify the style for pointers in the same data series respectively in the [Style List](https://devnet.logianalytics.com/hc/en-us/articles/1500010068061-Style-List-Dialog) dialog.

   In the Pointer Color box,

   * To use the [color defined for the ranges](#DialRangeColor) as the pointer color, check **Use Range Color**.
...
5. In the Target tab, specify the properties of the target in the dial gauge.
   ...
   ![Format Dial Gauge dialog - Target](https://devnet.logianalytics.com/hc/article_attachments/4404909210903/fmtdlgauge_tgt.gif)

   Check **Use Target Value** if you want to use target value for the dial gauge, and specify the target value as required. You can input the value in the text box directly or [use a formula to control the value](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties).
...
6. In the Frame tab, specify properties for the frame of the dial gauge.
   ...
   ![Format Dial Gauge dialog - Frame](https://devnet.logianalytics.com/hc/article_attachments/4404901242903/fmtdlgauge_frm.gif)
...
7. In the Color Range tab, select in the corresponding cells to set the minimum and maximum values, colors from the [color palette](https://devnet.logianalytics.com/hc/en-us/articles/1500010035022-Menu-Tabs#Palette), and names for the three predefined value ranges in each dial to fill the dials with colors. To add a new color range, select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404901125271/btn_add.gif), and if a color range is not required, select ![Remove button](https://devnet.logianalytics.com/hc/article_attachments/4404909120791/btn_rmv.gif) to remove it. Then in the Others box, specify the name and color for the values that do not fall into any of the ranges you define.

   ![Format Dial Gauge dialog - Range Color](https://devnet.logianalytics.com/hc/article_attachments/4404901243159/fmtdlgauge_rgclr.gif)
8. In the Hint tab, specify whether to
   include the category and series values and whether to scale big and small numbers in the gauge hint. A hint displays the value a pointer in the dial gauge represents when the mouse pointer points at the pointer in Logi JReport Designer view mode, in HTML result, or at server runtime. To make the hint shown, you need to make sure the [Show Tips](https://devnet.logianalytics.com/hc/en-us/articles/1500010033982-Chart-Paper-Properties#ShowTips) property in the Report Inspector is set to true.

   ![Format Dial Gauge dialog - Hint](https://devnet.logianalytics.com/hc/article_attachments/4404901243415/fmtdlgauge_hint.gif)
9. For a dial gauge chart in a library component, you can define web behaviors on it in the Behaviors tab.

   ![Format Dial Gauge dialog - Behaviors](https://devnet.logianalytics.com/hc/article_attachments/4404909211287/fmtdlgauge_bhvr.gif)
...
1. Right-click any circle in the activity gauge and select **Format Solid Gauge** on the shortcut menu, or double-click any circle in the activity gauge. The [Format Activity Gauge](https://devnet.logianalytics.com/hc/en-us/articles/1500010066201-Format-Activity-Gauge-Dialog) dialog displays.

   ![Format Activity Gauge dialog - Circular Graph](https://devnet.logianalytics.com/hc/article_attachments/4404901258903/fmtactvtygauge_crculr.gif)
2. In the Circular Graph tab, specify properties for the circles in the activity gauge.
   ...
   * To use the [color defined for the ranges](#ActivityRangeColor) as the circle color, check **Use Range Color**.
   * To specify color for the circles separately, leave Use Range Color uncheck. First make a choice for the Self Settings checkbox: when Self Settings is unchecked, the color pattern specified here will be synchronized to the [Pattern List property](https://devnet.logianalytics.com/hc/en-us/articles/1500010069721-Chart-Properties#Pattern) on the chart object in the Report Inspector, which can also be applied by data markers of other subtypes if the chart is a combo chart; when Self Settings is checked, it indicates the color pattern is private to the current data markers themselves (the circles in this case), which can be remembered and applied to the data markers of a new type automatically if later you change the type of the chart. Then specify the color and the transparency of the color schema to fill the circles in the same data series as the circle you have selected on to open the Format Activity Gauge dialog, or select the **Color List** button to specify the color pattern for circles in the same data series respectively in the [Color List](https://devnet.logianalytics.com/hc/en-us/articles/1500010030022-Color-List-Dialog) dialog. In the Background textbox, set the background color of the circles. To use colors that are lighter than the circles automatically as the background color, check **Auto**, then Background is disabled.

     In the Value box, specifies the minimum and maximum values to be displayed in the chart or [use a formula to control the property](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties).
3. In the Pointer tab, specify whether to show values for the pointers and the position relationship between the values and the pointers. You can select the position from the drop-down list or select **customized** to customize the position by dragging any pointer value in the design area.
   ...
   ![Format Activity Gauge dialog - Pointer](https://devnet.logianalytics.com/hc/article_attachments/4404901259159/fmtactvtygauge_pnt.gif)
4. In the Target tab, check **Use Target Value** if you want to use target value for the activity gauge, and specify the target value as required. You can input the value in the text box directly or [use a formula to control the value](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties).
   ...
   ![Format Activity Gauge dialog - Target](https://devnet.logianalytics.com/hc/article_attachments/4404901259415/fmtactvtygauge_tgt.gif)
5. In the Frame tab, specify properties for the frame of the activity gauge.

   ![Format Activity Gauge dialog - Frame](https://devnet.logianalytics.com/hc/article_attachments/4404909220631/fmtactvtygauge_frm.gif)
...
6. In the Range Color tab, select in the corresponding cells to set the minimum and maximum values, colors from the [color palette](https://devnet.logianalytics.com/hc/en-us/articles/1500010035022-Menu-Tabs#Palette), and names for the three predefined value ranges in each circle to fill the circles with colors. To add a new color range, select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404901125271/btn_add.gif), and if a color range is not required, select ![Remove button](https://devnet.logianalytics.com/hc/article_attachments/4404909120791/btn_rmv.gif) to remove it. Then in the Others box, specify the name and color for the values that do not fall into any of the ranges you define.

   ![Format Activity Gauge dialog - Range Color](https://devnet.logianalytics.com/hc/article_attachments/4404901259671/fmtactvtygauge_rgclr.gif)
7. In the Hint tab, specify whether to
   include the category and series values and whether to scale big and small numbers in the gauge hint. A hint displays the value a data range in a circle of the activity gauge represents when the mouse pointer points at the range in Logi JReport Designer view mode, in HTML result, or at server runtime. To make the hint shown, you need to make sure the [Show Tips](https://devnet.logianalytics.com/hc/en-us/articles/1500010033982-Chart-Paper-Properties#ShowTips) property in the Report Inspector is set to true.

   ![Format Activity Gauge dialog - Hint](https://devnet.logianalytics.com/hc/article_attachments/4404901259927/fmtactvtygauge_hint.gif)
8. For an activity gauge chart in a library component, you can define web behaviors on it in the Behaviors tab.

   ![Format Activity Gauge dialog - Behaviors](https://devnet.logianalytics.com/hc/article_attachments/4404901260311/fmtactvtygauge_bhvr.gif)
...
1. Right-click any data marks in a scale of the bar gauge chart and select **Format Bar Gauge** on the shortcut menu, or double-click any arc in the bar gauge. The [Format Bar Gauge](https://devnet.logianalytics.com/hc/en-us/articles/1500010066261-Format-Bar-Gauge-Dialog) dialog.

   ![Format Bar Gauge dialog - Staff Graph](https://devnet.logianalytics.com/hc/article_attachments/4404901254167/fmtbargauge_stfgraph.gif)
2. In the Staff Graph tab, specify properties for bars in the bar gauge.
   ...
   Check **Show Range Name** if you want to show the names of the ranges you have defined, then specify the font formats and effects of the range names in the Font and Effect boxes.
3. Go to the Axis tab to specify properties of the chart axis.
   1. In the Axis sub tab, set properties of the axis.

      ![Format Bar Gauge dialog - Axis - Axis1](https://devnet.logianalytics.com/hc/article_attachments/4404909217815/fmtbargauge_axis_axis1.gif)

      Check **Show Axis** to display axis in the bar gauge, then in the Type box, specify the position of the axis, which can be on the top or bottom of the bar, or in the center of the bar.
      ...
      In the Line box, specify the color and transparency of the axis or check **Use Range Color** to [use the color defined for the ranges](#BarRangeColor) as the axis color. Specify the style and thickness of the axis.
   ...
   2. In the Tick Mark sub tab, specify properties of the tick marks on the axis.
      ...
      To display tick marks on the axis, check how to show it in the Type box: on the top or bottom of the axis, or in the center of the axis.
   ...
   3. In the Label sub tab, specify properties for the data labels on the axis.
      ...
      * **Min and Max Values**  
      ...
      In the Gap box, specify the distance between the data label and the axis in pixels and whether to adjust the labels automatically to make them placed best. When Best Effect is checked, some labels will be hidden when they are overlapped.
   ...
   4. In the Format sub tab, specify the data format of the data labels on the axis.
      ...
      Select a category from the Category box, then select a format from the Format box and select **Add** to add it to the Stack box. If the formats listed in the Format box cannot meet your requirement, define the format in the Properties text box and select **Add** to add it as the format of the selected category. For the Number category, you can specify whether to scale big and small numbers by setting the [Auto Scale in Number](https://devnet.logianalytics.com/hc/en-us/articles/1500010066261-Format-Bar-Gauge-Dialog#AutoScale) option. You can add more than one format, but for each category only one format can be added. In the event that a format is not necessary, select it in the Stack box select **Remove** to clear it.
4. In the Pointer tab, specify the properties for the pointers on the bar gauge.
   ...
   In the Pointer Style box (available only when Use Pointer is checked), specify the style of the value pointer, the width and height of the pointer, the position relationship of the pointer and the bar, and the distance between the pointer and the bar in pixels. If required, select **Style List** to specify the style for pointers in the same data series respectively in the [Style List](https://devnet.logianalytics.com/hc/en-us/articles/1500010068061-Style-List-Dialog) dialog.

   In the Pointer Color box,

   * To use the [color defined for the value ranges](#BarRangeColor) as the pointer color, check **Use Range Color**.
...
5. In the Target tab, specify properties of the target in the bar gauge.
   ...
   Check **Use Target Value** if you want to use target value for the bar gauge, and specify the target value as required. You can input the value in the text box directly or [use a formula to control the value](https://devnet.logianalytics.com/hc/en-us/articles/1500010033202-Using-Formulas-to-Control-Object-Properties).
...
7. In the Range Color tab, specify whether to use gradient effect, select in the corresponding cells to specify the minimum and maximum values, colors from the [color palette](https://devnet.logianalytics.com/hc/en-us/articles/1500010035022-Menu-Tabs#Palette), and names for the three predefined value ranges in each bar to fill the bars with colors. To add a new color range, select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404901125271/btn_add.gif), and if a color range is not required, select ![Remove button](https://devnet.logianalytics.com/hc/article_attachments/4404909120791/btn_rmv.gif) to remove it. Then in the Others box, specify the name and color for the values that do not fall into any of the ranges you define.

   ![Format Bar Gauge dialog - Rane Color](https://devnet.logianalytics.com/hc/article_attachments/4404901255447/fmtbargauge_rgclr.gif)
8. In the Hint tab, specify whether to
   include the category and series values and whether to scale big and small numbers in the gauge hint. A hint displays the value a pointer in the bar gauge represents when the mouse pointer points at the pointer in Logi JReport Designer view mode, in HTML result, or at server runtime. To make the hint shown, you need to make sure the [Show Tips](https://devnet.logianalytics.com/hc/en-us/articles/1500010033982-Chart-Paper-Properties#ShowTips) property in the Report Inspector is set to true.
...
1. Right-click any bubble in the bubble gauge chart and select **Format Bubble Gauge** on the shortcut menu, or double-click any bubble in the bubble gauge. The [Format Bubble Gauge](https://devnet.logianalytics.com/hc/en-us/articles/1500010066301-Format-Bubble-Gauge-Dialog) dialog displays.
...
2. In the Staff Graph tab, set the properties for the bubbles in the chart.
   ...
   In the Border box, check **Show Border** if you want to show the border of the bubbles, then set properties of the border including the line style, thickness, color and color transparency.
...
4. In the Range Color tab, select in the corresponding cells to specify the minimum and maximum values, colors from the [color palette](https://devnet.logianalytics.com/hc/en-us/articles/1500010035022-Menu-Tabs#Palette) and names for the three predefined value ranges in each bubble to fill the bubbles with colors. To add a new color range, select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404901125271/btn_add.gif), and if a color range is not required, select ![Remove button](https://devnet.logianalytics.com/hc/article_attachments/4404909120791/btn_rmv.gif) to remove it. Then in the Others box, specify the name and color for values that do not fall into any of the range you define.
...
5. In the Hint tab, specify whether to
   include the category and series values and whether to scale big and small numbers in the gauge hint. A hint displays the value a bubble in the bubble gauge represents when the mouse pointer points at the bubble in Logi JReport Designer view mode, in HTML result, or at server runtime. To make the hint shown, you need to make sure the [Show Tips](https://devnet.logianalytics.com/hc/en-us/articles/1500010033982-Chart-Paper-Properties#ShowTips) property in the Report Inspector is set to true.
...
3. In the Font tab, format the font of the labels, including the font face, size, color, transparency, rotation angle, shearing angle, and effects such as style, strikeout, underline and so on.
   ...
   ![Format Pointer Label dialog - Font](https://devnet.logianalytics.com/hc/article_attachments/4404909202583/fmtpntlbl_font.gif)
4. In the Format tab, specify the data format of the labels.

   ![Format Pointer Label dialog - Behaviors](https://devnet.logianalytics.com/hc/article_attachments/4404901226263/fmtpntlbl_fmt.gif)

...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009778781-Going-Through-Web-Report-Data
updated_at: 2021-07-24T00:47:32Z
---

# Going Through Web Report Data

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404880134167/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009750082-Using-Dynamic-Resources-and-Local-Parameters-in-Web-Report-)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404880134423/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009778741-Applying-Filters-in-Web-Report)
...
1. Right-click any region on the category axis, Asia-Pacific for example, and choose **Go To** from the shortcut menu. Server lists the groups available for the action on the submenu, which are all the other group objects in the business view used by the chart.
   ...
   ![Go To Submenu](https://devnet.logianalytics.com/hc/article_attachments/4404880181783/wbrpt_edt_go1.gif)
2. Select **State** on the submenu. In the regenerated result, we can see that **State** becomes the group for categories and **Sales Year** remains the group for series.
   ...
   ![Go To State](https://devnet.logianalytics.com/hc/article_attachments/4404880182295/wbrpt_edt_go2.gif)
...
1. Go back to the original chart in the above example by selecting the **Undo** button ![Undo button](https://devnet.logianalytics.com/hc/article_attachments/4404880138391/btn_undo.gif) on the toolbar.
2. Right-click **Asia-Pacific** on the category axis, and point to **Go to By Value** on the shortcut menu. Server displays a submenu for the command and lists the same items as those of Go To.
3. Select **State** too. The result is as follows:

   ![Go To By State](https://devnet.logianalytics.com/hc/article_attachments/4404880182551/wbrpt_edt_go3.gif)
   ...
   ![Go To Filter](https://devnet.logianalytics.com/hc/article_attachments/4404880183319/wbrpt_edt_go4.gif)
...
1. Go back to the original chart in the above example by selecting the **Undo** button ![Undo button](https://devnet.logianalytics.com/hc/article_attachments/4404880138391/btn_undo.gif) on the toolbar.
2. Right-click the value **Asia-Pacific**, on the shortcut menu, point to **Go Down**. Server lists **Country** as the submenu item.

   ![Go Down Submenu](https://devnet.logianalytics.com/hc/article_attachments/4404885338263/wbrpt_edt_godwn1.gif)
3. Select **Country**. The chart now displays the data about countries in the Asia Pacific region.

   ![Go Down to Country](https://devnet.logianalytics.com/hc/article_attachments/4404880183959/wbrpt_edt_godwn2.gif)
4. The one-level-lower group for Country defined in the hierarchy is State. Right-click on **China** and select **Go Down** > **State**. The chart now shows states in China.
   ...
   ![Go Down to State](https://devnet.logianalytics.com/hc/article_attachments/4404880184343/wbrpt_edt_godwn3.gif)
   ...
   ![Go To Filter](https://devnet.logianalytics.com/hc/article_attachments/4404880184727/wbrpt_edt_godwn4.gif)

   This is because, when you perform a go-down action, Server creates a filter based on the value you select on. In this example, we first select on the Asia-Pacific region, so Server drills this region one level down to display countries in Asia Pacific, and thus creates the filter Region = Asia-Pacific. If you want all data in the one-level-lower group to display when you drill down a group, you can remove the corresponding filter from the "Go To" Filter panel, by selecting **X** beside the filter condition.
...
1. Based on the report result after go-down, right-click any value of State, Beijing for example, on the shortcut menu, point to **Go Up**, and we can see that Server lists **Country** as the submenu item.

   ![Go Up Submenu](https://devnet.logianalytics.com/hc/article_attachments/4404880184983/wbrpt_edt_goup.gif)
2. Select **Country** to update the chart. Country is now the group for categories, and Server removes the filter condition Country = China from the "Go To" Filter panel.
   ...
   ![Go Up to Country](https://devnet.logianalytics.com/hc/article_attachments/4404880183959/wbrpt_edt_godwn2.gif)
3. The one-level-higher group for Country defined in the hierarchy is Region. Now right-click any value of Country, Australia for example, on the shortcut menu, select **Go Up** > **Region**. The chart restores to the original state.
   ...
   ![Go Up to Region](https://devnet.logianalytics.com/hc/article_attachments/4404880181527/wbrpt_edt_go.gif)
...
1. Right-click the bar represents the total sales of Asia-Pacific in 2016 and select **Go to Detail** on the shortcut menu.
   ...
   ![Go To Detail](https://devnet.logianalytics.com/hc/article_attachments/4404880185367/wbrpt_edt_gotodtl1.gif)
2. Server displays the detail table of the summary value in the same frame or a new window, according to the setting of [Target of Detail Table Report and Links](https://devnet.logianalytics.com/hc/en-us/articles/1500009750242-Customizing-Web-Report-Studio-Profile#Target) in the Web Report Studio profile. By default, the detail table contains all the group and detail objects in the same business view category as the aggregate object from which the summary value is generated.

   ![Detail Table](https://devnet.logianalytics.com/hc/article_attachments/4404885339543/wbrpt_edt_gotodtl2.gif)
3. If Server displays the detail table in the same frame as the original report, you can select the **Back** button ![Back button](https://devnet.logianalytics.com/hc/article_attachments/4404880165655/btn_back.gif) on the toolbar to go back to the original report.
...
1. Right-click any summary value (for a chart it is any data marker) and select **Edit Detail Table** from the shortcut menu. Server displays the [Edit Detail Table](https://devnet.logianalytics.com/hc/en-us/articles/1500009746962-Edit-Detail-Table-Dialog-Box-Properties) dialog box.
...
3. You can use the **Sort** button ![Sort button](https://devnet.logianalytics.com/hc/article_attachments/4404880135447/btn_sort.gif) and the **Search** button ![Search button](https://devnet.logianalytics.com/hc/article_attachments/4404880135831/btn_srch.gif) to sort and search for resources in the left box.
4. To adjust the order of the added fields in the right box, select the **Up** button![Move Up button](https://devnet.logianalytics.com/hc/article_attachments/4404885326231/btn_mvup.gif) or the **Down** button ![Move Down button](https://devnet.logianalytics.com/hc/article_attachments/4404880154263/btn_mvdown.gif).
5. From the **Target** drop-down list, select the window or frame in which to load the detail table.
   ...
   * **<Server Setting>**  
      Server loads the detail table according to the setting of the [Target of Detail Table Report and Links](https://devnet.logianalytics.com/hc/en-us/articles/1500009750242-Customizing-Web-Report-Studio-Profile#Target) option in the Web Report Studio profile.

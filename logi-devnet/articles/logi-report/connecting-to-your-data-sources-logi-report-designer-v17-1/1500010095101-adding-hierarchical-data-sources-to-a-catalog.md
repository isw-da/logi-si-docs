...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500010095101-Adding-Hierarchical-Data-Sources-to-a-Catalog
updated_at: 2021-07-23T19:14:53Z
---

# Adding Hierarchical Data Sources to a Catalog

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404856790679/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500010095121-Hierarchical-Data-Source-API)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404856790935/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500010095141-Developing-Reports-from-Hierarchical-Data-Sources)

# Adding Hierarchical Data Sources to a Catalog

This topic introduces the two methods for adding hierarchical data sources to a catalog according to how you implement the HDS API: adding general hierarchical data sources and importing hierarchical data sources from an XML file.
...
2. In the Catalog Manager, right-click the node of a data source and choose **New General Hierarchical Data Source** from the shortcut menu.
   ...
   If you want to add the HDS to a new data source in the catalog, select any of the existing catalog data source, select **New Data Source** on the Catalog Manager toolbar, then in the New Data source dialog box, specify the name of the data source, select the **Hierarchical** connection type and select **OK**.
3. In the [New General Hierarchical Data Source dialog box](https://devnet.logianalytics.com/hc/en-us/articles/1500010098021-New-General-Hierarchical-Data-Source-Dialog-Box), select the **Browse** button to specify the HDS class name.
...
4. In the **Parameter** box, type a number or parameter string. The parameter string must match the format defined in the [HDS class](https://devnet.logianalytics.com/hc/en-us/articles/1500010095121-Hierarchical-Data-Source-API#ParamString).

   You can reference [parameters](https://devnet.logianalytics.com/hc/en-us/articles/1500010099721-Working-with-Parameters) and [constant level formulas](https://devnet.logianalytics.com/hc/en-us/articles/1500010061022-Formula-Levels#CLF) predefined in the current catalog data source in the format *@FieldName*, and the special field "[User Name](https://devnet.logianalytics.com/hc/en-us/articles/1500010094881-Working-with-Special-Fields#UserName)" as *@username* in the PARAMETER string.
5. Select **Load Tree**. Designer then parses the data source tree. Modify the column properties in the **Columns** box.
...
6. In the New General Hierarchical Data Source dialog box, select the **Browse** button to find the class HierarchicalDataSource.class.
...
3. In the [New XML Hierarchical Data Source dialog box](https://devnet.logianalytics.com/hc/en-us/articles/1500010098121-New-XML-Hierarchical-Data-Source-Dialog-Box), select the **Browse** buttons to specify the XML URI and XSD URI as required. Select the **Load Tree** button to load the structure of the XML file. Designer then lists the root of the file in the **Root Name** text box.
   ...
   * Designer supports static string inline with multiple parameters, for example:
     ...
     It is the URL used to run the sample report "Key Performance Indicators Report.cls" in XML on Server. Type the URL into the **XML URI** text box, and select the **Load Tree** button. Designer then loads the structure of the returned XML stream in the **Structure** box (before doing so, make sure that Server is started). Modify the column properties in the **Columns** box as required.

     You can use the colon ":" and At "@" symbols to identify Logi Report parameter names. If you use these symbols in your XML URI but you do not want Logi Report to parse them as parameters, you must add quotation marks to them. For example, when you browse to `d:\test\employee.xml`, you can quote it either as `"d:\test\employee.xml"` or `d":"\test\employee.xml`.

     ![Note icon](https://devnet.logianalytics.com/hc/article_attachments/4404856814359/noteicon.jpg) The Server administrator must select the No Security Checking option in the Server Console > Administration > Configuration > Advanced page, so that Server can parse the URL successfully. In addition, you can reference the Logi Report parameters in the URL for setting different values dynamically at runtime.
...
When you import an XML HDS with an XSD file, the XML file only provides the data to the Designer reports, while the structure, data type, and so on of the data from the XML file is defined in the XSD file. That is, the XSD file determines the structure of the HDS. You should be aware of the following points about the XSD file in order to generate a correct report based on an XML HDS with an XSD file.

**Data type conversion**

Before the data type defined in the XSD file can function with Designer, Designer should at first convert the data type into a corresponding data type when you import the XML hierarchical data source based on the rules in the following conversion table.
...
* **The ComplexType**
  ...
  ![ComplexType diagram](https://devnet.logianalytics.com/hc/article_attachments/4404857029527/cnctn_xml_xsd1.gif)
...
* **The SimpleType**
  ...
  ![SimpleType diagram](https://devnet.logianalytics.com/hc/article_attachments/4404857029783/cnctn_xml_xsd2.gif)
...
* **The Element**
  ...
  ![Element diagram](https://devnet.logianalytics.com/hc/article_attachments/4404857030039/cnctn_xml_xsd3.gif)
...
* When you specify a schema file for the XML data source file, due to the schema file being complex, there are some limitations:
  + **Namespace**: Designer supports the default namespace (3w) and target namespace. You can define a prefix for the default namespace, such as xs or xsd, but the value of elementFormDefault must be qualified. The value of attributeFormDefault should be unqualified, but Designer does not allow you to add prefixes before the attributes of the elements.
  + **Type**: If you want to use a customized [complexType](#Complex) or [simpleType](#Simple), do not add the prefix to the value of the type, while if you use the built-in simpleType or complexType, you must add the prefix.

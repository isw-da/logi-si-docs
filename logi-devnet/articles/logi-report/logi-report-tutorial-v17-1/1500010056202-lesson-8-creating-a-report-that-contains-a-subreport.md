...
4. In the Data screen of the Banded Wizard dialog box, expand the **Imported SQLs** node in Data Source 1 and select **OrdersReport**.

   ![Select Data](https://devnet.logianalytics.com/hc/article_attachments/4404848767895/rpt_lsn8_slctsql.gif)
5. Select **Style** to switch to the screen and select the **Simple** style for the report, then select **Finish**.
   ...
   ![Banded Panels](https://devnet.logianalytics.com/hc/article_attachments/4404848768151/rpt_lsn8_bndrpt.gif)
...
2. In the Group screen of the Banded Wizard dialog box, add the DBField **Orders\_OrderID** as the group-by field by selecting it and selecting ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404848768535/btn_addarw.gif), then select **Finish**.

   ![Add Group](https://devnet.logianalytics.com/hc/article_attachments/4404857139223/rpt_lsn8_fmtgrp.gif)
...
5. Align the newly-added objects and resize them in the GH panel as follows:
   ...
   ![Add Fields to Banded Object](https://devnet.logianalytics.com/hc/article_attachments/4404857139735/rpt_lsn8_algn.gif)
6. Resize the group-by field in the GH panel, select **Insert** > **Label** to add a label ahead of it and edit its text to "**Order ID:**".

   ![Add Label](https://devnet.logianalytics.com/hc/article_attachments/4404857139991/rpt_lsn8_id.gif)
...
10. Format labels and DBFields in the report to improve their appearance. Here, we will not describe the formatting process in detail, you can refer to [Task 3 of Lesson 1](https://devnet.logianalytics.com/hc/en-us/articles/1500010056142-Lesson-1-Creating-a-Standard-Banded-Report#Task3) for assistance.

    Then the first report tab in the report is created to show the order detail information as follows in design mode:

    ![Main Report](https://devnet.logianalytics.com/hc/article_attachments/4404857140247/rpt_lsn8_frstrpt.gif)

    And the report looks as follows in view mode:

    ![Preview the Main Report](https://devnet.logianalytics.com/hc/article_attachments/4404857140503/rpt_lsn8_view.gif)
...
3. In the Tabular Wizard dialog box, keep the default settings and select **Finish**. A report tab with a blank 2\*2 tabular is created.
...
1. Select the first tabular cell and select **Insert** > **Chart** on the menu bar. Designer displays the Create Chart dialog box.
2. In the Data screen, select the **More Options** button, select the **Existing Dataset** radio button, then select the dataset **OrdersReport**. Select **Next**.
3. In the Type screen, keep the default chart type Clustered Bar 2-D and select **Next**.
4. In the Display screen, select the summary **Sum\_SQLProductSalesbyOrderID** and select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404848768535/btn_addarw.gif) beside the Show Values box, Orders\_OrderID is then added automatically to the Category box.
...
6. When a box attached to your mouse pointer, select the tabular cell to place the chart there.
   ...
   ![Insert Chart](https://devnet.logianalytics.com/hc/article_attachments/4404848769943/rpt_lsn8_box.gif)
...
![Add Label to Chart](https://devnet.logianalytics.com/hc/article_attachments/4404857140759/rpt_lsn8_varyclr.gif)

9. Right-click the bar chart and select **Add Label** from the shortcut menu. A label is then added to the chart.

   ![Add Label to Chart](https://devnet.logianalytics.com/hc/article_attachments/4404848770199/rpt_lsn8_addlbl.gif)
...
12. Switch to the **Font** tab, change Font Size to **12pt**, Font Color to **808080** and select **OK**.
    ...
    ![Format Label](https://devnet.logianalytics.com/hc/article_attachments/4404857141015/rpt_lsn8_fmtlbl.gif)
...
15. In the Report Inspector, select the **Chart Object** and **Chart Object 1** nodes from the report structure tree by holding the Ctrl key on the keyboard, then change the Border Type properties to **none**.
    ...
    ![Edit Border Type](https://devnet.logianalytics.com/hc/article_attachments/4404857141271/rpt_lsn8_border.gif)
16. Select the **Chart Legend Object** and **Chart Legend Object 1** nodes and change the Border Type property to **None** as well.
17. Further format the two charts to improve their appearance if desired. For more information about formatting chart, see [Task 2 of Lesson 4](https://devnet.logianalytics.com/hc/en-us/articles/1500010093221-Lesson-4-Creating-a-Chart-Report#Task2).

    Now the second report tab in the report is created to show the sales total by orders.

    ![Subreport](https://devnet.logianalytics.com/hc/article_attachments/4404848770455/rpt_lsn8_scdrpt.gif)
...
After creating the report, we can insert the subreport to the primary report by specifying relationships between them. In this lesson, we can specify different relationships between the primary report (the first one) and the two chart components in the second report tab by setting up different links between them. Then Designer will build a subreport for every order with the conditions, so that it is convenient to see the corresponding customer and employee information of every order.
...
1. Select **OrderDetailInformation** on the report tab bar to switch to the report tab.
   ...
   ![Report Tab Bar](https://devnet.logianalytics.com/hc/article_attachments/4404848770711/rpt_lsn8_bar.gif)
...
4. When Designer attaches a box to the mouse pointer, select the newly-added panel to place the subreport into it. Designer displays the Subreport dialog box.
...
7. Select the **Add** button beside the Component in Report Tab box in the Field tab, choose **Chart Object** in the Choose Component dialog box, then select **OK**.
   ...
   ![Add Component from Subreport](https://devnet.logianalytics.com/hc/article_attachments/4404857142039/rpt_lsn8_subrpt.gif)
8. Build the relationship between the primary report and the selected chart object on the Orders\_CustomerID field by selecting the field and selecting ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404848768535/btn_addarw.gif).  

   ![Edit Subreport Relationship](https://devnet.logianalytics.com/hc/article_attachments/4404848770967/rpt_lsn8_rltnshp.gif)
...
11. Select **OK** in the Subreport dialog box and the report tab will be inserted in the OrderDetails report tab as follows:
    ...
    ![Insert Subreport](https://devnet.logianalytics.com/hc/article_attachments/4404848771479/rpt_lsn8_instsub.gif)
...
2. Select the **Filter** tab, select the **Add Condition** button to add a filter line, select **ORDERS\_ORDERID** from the field drop-down list, **<=** from the operator drop-down list, and type **3050** in the value text box to specify the filter condition as **ORDERS\_ORDERID <= 3050**. Select **OK** to apply the filter.

   ![Filter Dataset](https://devnet.logianalytics.com/hc/article_attachments/4404857142295/rpt_lsn8_fltr.gif)
...
4. Select the **View** tab, Designer displays the report tab that contains a subreport as follows: the primary report shows the orders detail information grouped by order ID, and the subreport shows the orders information by different customers and different employees respectively in the two charts.
   ...
   ![Preview Report](https://devnet.logianalytics.com/hc/article_attachments/4404848771735/rpt_lsn8_rpt.gif)

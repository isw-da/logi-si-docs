...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009771421-Setting-Up-and-Configuring-a-Logi-Report-Server-Cluster
updated_at: 2021-07-24T00:49:31Z
---

# Setting Up and Configuring a Logi Report Server Cluster

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404880134167/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009771341-Setting-Up-and-Starting-a-Logi-Report-Server-Cluster)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404880134423/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009743462-Starting-a-Logi-Report-Server-Cluster)
...
4. In the **Service** tab of the Configure System Environment screen, select the **Network Address At** option and type the IP address of the server.
   ...
   ![Installation - Service tab](https://devnet.logianalytics.com/hc/article_attachments/4404880575639/clstr_set_set_svc.gif)
...
6. Clear the **Disable Cluster** option at the bottom.
   ...
   ![Installation - Cluster tab](https://devnet.logianalytics.com/hc/article_attachments/4404880576023/clstr_set_set_clstr.gif)
7. From the Load Balancer Type drop-down list, select the built-in [algorithm](https://devnet.logianalytics.com/hc/en-us/articles/1500009771301-Logi-Report-Server-Cluster-Main-Features#Algorithms) for load balancing clustered servers: Min-load, Round Robin, Weighted Min-load, or Random.
...
2. [Log onto its Server Console](https://devnet.logianalytics.com/hc/en-us/articles/1500009777061-Accessing-Logi-Report-Server-via-a-Web-Browser).
...
6. Select **Save** to enable the cluster.
   ...
   Logi Report automatically generates a cluster member ID for the server. If you need to modify it, you can open **server.properties** in `<install_root>\bin` and set the property **cluster.member.id**. However, you should not change the auto generated cluster member ID in a Logi Report Server Cluster, because distributed storage uses the member ID to recognize on which clustered server it stores the physical files.
...
9. Point to **Administration** on the system toolbar, select **Configuration** > **Cluster** > **Configuration** from the drop-down menu to open the Configuration page.
   ...
   ![Cluster - Configuration page](https://devnet.logianalytics.com/hc/article_attachments/4404880578967/clstr_set_set_config.gif)
...
* The databases that can be used as the server database in a Logi Report Server Cluster must be able to support the lock function, therefore, you cannot use HSQLDB as the server's database in a Logi Report Server Cluster because HSQLDB does not support the function.
...
* If you want to use resources from [real paths](https://devnet.logianalytics.com/hc/en-us/articles/1500009777261-Getting-and-Using-Resources-From-a-Real-Path) in a Logi Report Server Cluster, you should first specify a disk in your file system and make sure that every clustered server in the cluster has the same disk name mapping to the same physical location.
* In order to fax report result in a cluster, you need to [configure the fax settings](https://devnet.logianalytics.com/hc/en-us/articles/1500009743702-Configuring-Export-Settings#Fax) for each clustered server respectively.
* When a cluster is enabled, Server also saves the following properties and their mapping UI options into the system database.

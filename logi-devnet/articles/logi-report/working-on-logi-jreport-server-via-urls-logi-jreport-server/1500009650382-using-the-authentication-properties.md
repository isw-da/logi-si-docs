...
* **jrs.authorization**  
  ...
  + `http://localhost:8888/Logi JReport/dashboard/app/entry/run.jsp?jrd_resext={"active":0,"reslst":[{"name":"/SampleReports/Coffee Sales Dashboard.dsh","ver":"-1"}]}&jrs.authorization=YWRtaW46YWRtaW4%3D`
...
* **jrs.auth\_uid and jrs.auth\_pwd**  
  ...
  + This URL requests the JSP page viewVersion.jsp with credentials passed in to log in user "scott" with password "tiger":  
    `http://localhost:8888/jinfonet/viewVersion.jsp?jrs.cmd=jrs.view_ver_def&jrs.rst_version=1&jrs.ver_suff=.pdf&jrs.report=/InvoiceReport.cls&jrs.auth_uid=scott&jrs.auth_pwd=tiger`
  + This URL runs the dashboard with credentials passed in to log in user "scott" with password "tiger":  
    `http://localhost:8888/Logi JReport/dashboard/app/entry/run.jsp?jrd_resext={"active":0,"reslst":[{"name":"/SampleReports/Coffee Sales Dashboard.dsh","ver":"-1"}]}&jrs.auth_uid=scott&jrs.auth_pwd=tiger`

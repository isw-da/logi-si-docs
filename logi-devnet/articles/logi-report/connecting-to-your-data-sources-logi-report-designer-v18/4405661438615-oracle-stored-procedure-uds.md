...
1. [Create a catalog](https://devnet.logianalytics.com/hc/en-us/articles/4405661330711-Creating-Opening-and-Saving-Catalogs) or [open a catalog](https://devnet.logianalytics.com/hc/en-us/articles/4405661330711-Creating-Opening-and-Saving-Catalogs#Open) to which you want to add the Oracle stored procedure.
...
* **valueX**  
  The value of the parameters separated by ',' and the data types must correspond to the ones defined in the DBMS. If valueX should contain a set of constant string values, for example, 10, 20, and 30 together serve as valueX, you need to use "\\" to escape the comma.
  ...
  Or, instead of typing the parameter values one by one in the preceding parameter string, you can also use the [parameters](https://devnet.logianalytics.com/hc/en-us/articles/4405661800215-Working-with-Parameters) and [constant level formulas](https://devnet.logianalytics.com/hc/en-us/articles/4405664602263-Formula-Levels#CLF) in the current catalog data source to substitute the parameter values, then Designer automatically adds "\\" to escape the comma. For example:
...
5. In the **Parameters** box, type one of the following:
   ...
   * `OWNER=SCOTT&PROCNAME=empquery&SQL={call empquery(?,?)}&REFCURSORINDEX=2&INPARAMVALUE="SALESMAN",varchar,1`
   ...
   ![New UDS Example](https://devnet.logianalytics.com/hc/article_attachments/4420556227991/cnctn_uds_orcl_eg1.gif)
6. Select **OK** to add the UDS class into the catalog.
...
5. In the **Parameter** box, type one of the following:
   ...
   * `OWNER=SCOTT&PROCNAME=demo_pkg.empquery_proc&SQL={call demo_pkg.empquery_proc(?,?)}&REFCURSORINDEX=2&INPARAMVALUE="SALESMAN",varchar,1`
   ...
   ![New UDS Example](https://devnet.logianalytics.com/hc/article_attachments/4420550960791/cnctn_uds_orcl_eg2.gif)
6. Select **OK** to add the UDS class into the catalog.
...
5. In the **Parameter** box, type one of the following:
   ...
   * `OWNER=SCOTT&PROCNAME=deptquery&SQL={? = call deptquery(?)}&REFCURSORINDEX=1&INPARAMVALUE=@p_dname,varchar,2`
   ...
   ![New UDS Example](https://devnet.logianalytics.com/hc/article_attachments/4420556228375/cnctn_uds_orcl_eg3.gif)
6. Select **OK** to add the UDS class into the catalog.
...
5. In the Parameter box, type one of the following:
   ...
   * `OWNER=SCOTT&PROCNAME=shdemo_pkg.deptquery_func&SQL={? = call shdemo_pkg.deptquery_func(?)}&REFCURSORINDEX=1&INPARAMVALUE="SALES",varchar,2`
   ...
   ![New UDS Example](https://devnet.logianalytics.com/hc/article_attachments/4420556228759/cnctn_uds_orcl_eg4.gif)
6. Select **OK** to add the UDS class into the catalog.

...
3. In the [New General Hierarchical Data Source dialog box](https://devnet.logianalytics.com/hc/en-us/articles/4405661657111-New-General-Hierarchical-Data-Source-Dialog-Box), select **Browse** to specify the HDS class name.

   ![New General Hierarchical Data Source dialog box](https://devnet.logianalytics.com/hc/article_attachments/4420394739735/nwhds.gif)
4. In the **Parameter** box, type a number or parameter string. The parameter string must match the format defined in the [HDS class](https://devnet.logianalytics.com/hc/en-us/articles/4405661420183-Hierarchical-Data-Source-API#ParamString).

   You can reference [parameters](https://devnet.logianalytics.com/hc/en-us/articles/4405661800215-Working-with-Parameters) and [constant level formulas](https://devnet.logianalytics.com/hc/en-us/articles/4405664602263-Formula-Levels#CLF) predefined in the current catalog data source in the format *@FieldName*, and the special field "[User Name](https://devnet.logianalytics.com/hc/en-us/articles/4405664402327-Working-with-Special-Fields#UserName)" as *@username* in the PARAMETER string.
...
3. In the [New XML Hierarchical Data Source dialog box](https://devnet.logianalytics.com/hc/en-us/articles/4405661661975-New-XML-Hierarchical-Data-Source-Dialog-Box), select **Browse** to specify the XML URI and XSD URI as required. Select **Load Tree** to load the structure of the XML file. Designer then lists the root of the file in the **Root Name** text box.

   ![New XML Hierarchical Data Source dialog box](https://devnet.logianalytics.com/hc/article_attachments/4420394739095/nwxmlhds.gif)

   Designer supports all kinds of URI as the XML data source.

   * If you want to specify a schema file for the XML data source file, you must first make sure that the schema file path specified in the XML data source file is consistent with the path you specified in the XSD URI field, and that this file actually exists. Select **Load Tree** to load the structure of the XML file. Designer then lists the root of the file in the **Root Name** text box. Modify the column properties in the **Columns** box as required.
   * Designer supports static string inline with multiple parameters, for example:
     ...
     It is the URL used to run the sample report "Key Performance Indicators Report.cls" in XML on Server. Type the URL into the **XML URI** text box, and select **Load Tree**. Designer then loads the structure of the returned XML stream in the **Structure** box (before doing so, make sure that Server is started). Modify the column properties in the **Columns** box as required.

     You can use colon ":" and the "@" symbols to identify Logi Report parameter names. If you use these symbols in your XML URI but you do not want Logi Report to parse them as parameters, you must add quotation marks to them. For example, when you browse to `d:\test\employee.xml`, you can quote it either as `"d:\test\employee.xml"` or `d":"\test\employee.xml`.

     ![Note icon](https://devnet.logianalytics.com/hc/article_attachments/4420394642839/noteicon.jpg) The Server administrator must select the No Security Checking option in the Administration > Configuration > Advanced page of the Server Console, so that Server can parse the URL successfully. In addition, you can reference the Logi Report parameters in the URL for setting different values dynamically at runtime.
...
* **The ComplexType**
  ...
  ![ComplexType diagram](https://devnet.logianalytics.com/hc/article_attachments/4420410861207/cnctn_xml_xsd1.gif)
...
* **The SimpleType**
  ...
  ![SimpleType diagram](https://devnet.logianalytics.com/hc/article_attachments/4420410861591/cnctn_xml_xsd2.gif)
...
* **The Element**
  ...
  ![Element diagram](https://devnet.logianalytics.com/hc/article_attachments/4420402556823/cnctn_xml_xsd3.gif)

...
![Report Sketch](https://devnet.logianalytics.com/hc/article_attachments/4404885680535/rpt_lsn7_sketch.gif)
...
3. In the Tabular Wizard dialog box, keep the default settings and select **Finish** to create a 2\*2 tabular.
   ...
   ![Tabular Wizard](https://devnet.logianalytics.com/hc/article_attachments/4404885680791/rpt_lsn7_tblr.gif)
...
4. In the Display screen, add the **Products\_Product ID** DBField to the Columns box by selecting it and selecting ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404880676375/btn_addarw.gif), add the **Region\_AbbreviationName** formula to the Rows box by selecting it and selecting ![Add Row button](https://devnet.logianalytics.com/hc/article_attachments/4404880680343/btn_addrow.gif), add the **Total** formula to the Summaries box by selecting it and selecting ![Add Summary button](https://devnet.logianalytics.com/hc/article_attachments/4404885681047/btn_addsum.gif), and then double-click in the Aggregate text box and select **Sum** from the drop-down list as the aggregate function of the summary.

   ![Add Fields to Crosstab](https://devnet.logianalytics.com/hc/article_attachments/4404880680727/rpt_lsn7_crstbdsply.gif)
5. Select **Style** to switch to the screen and apply the **Classic** style to the crosstab, then select  **Finish** to close the Create Crosstab dialog box and place the crosstab in the first tabular cell row.
   ...
   ![Insert Crosstab to Tabular Cell](https://devnet.logianalytics.com/hc/article_attachments/4404885681303/rpt_lsn7_crstb.gif)
...
2. In the Data screen, select the **More Options** button, then select the **Existing Dataset** radio button and select the dataset **ProductPerformance**. Select **Next**.
   ...
   ![Select Existing Dataset](https://devnet.logianalytics.com/hc/article_attachments/4404880680983/rpt_lsn7_dtset.gif)
...
4. In the Display screen, select the **Y Axis** node in the Color box, drag and drop the summary **Count\_QuantitybyProductID** under it, then select the **Radius** node and add **Sum\_ProductSalesbyProductID** under it. The summaries are grouped by the DBField ProductID\_FK1 so the field is added to the Bubble box automatically. Leave the Series box empty.
   ...
   ![Add Chart Fields](https://devnet.logianalytics.com/hc/article_attachments/4404880681495/rpt_lsn7_dsply.gif)
...
3. Select **Group** to switch to the screen, select the DBField **ProductID\_FK1** and select ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4404880676375/btn_addarw.gif) to add it as the group-by field.
4. Go to the **Style** screen and select the **Classic** style for the banded object. Select **Finish** to close the Create Banded Object dialog box and then select in TabularCell3 to place the banded object in it. The panels in the banded object are identified on the left side by their abbreviations: a banded header panel (BH), a banded page header panel (BPH), a group header panel (GH), a detail panel (DT), a group footer panel (GF), a banded page footer panel (BPF), and a banded footer panel (BF).
   ...
   ![Add Components to Report](https://devnet.logianalytics.com/hc/article_attachments/4404885681559/rpt_lsn7_cell.gif)
5. Drag the **Label** icon ![Label button](https://devnet.logianalytics.com/hc/article_attachments/4404885681815/btn_label.gif) from the Basic group in the Components panel to the BPH panel in the banded object, above the group-by field in the GH panel to label the field, then edit the label text to
   **Product ID**.
...
9. Right-click in the BH panel and select **Hide** from the shortcut menu to hide it from view. Repeat this to hide all panels that do not hold data.
   ...
   ![Edit Banded Object](https://devnet.logianalytics.com/hc/article_attachments/4404885682071/rpt_lsn7_fld.gif)
...
1. Select the **Dataset Filter** button ![Dataset Filter button](https://devnet.logianalytics.com/hc/article_attachments/4404880674199/btn_fltr.gif) on the toolbar of the Data panel.
2. In the Dataset Filter dialog box, select the **Add Condition** button to add a filter line, select **PRODUCTID\_FK1** from the field drop-down list, **<=** from the operator drop-down list, and type **10** in the value text field to specify the filter condition as **PRODUCTID\_FK1 <= 10**, then select **OK**.
   ...
   ![Filter Dataset](https://devnet.logianalytics.com/hc/article_attachments/4404880681751/rpt_lsn7_fltr.gif)
...
2. In the Font tab of the Format Text dialog box, set Font Size to **18**, Font Color to **Red** and Bold to **true**, then select **OK**.
   ...
   ![Format Report Title](https://devnet.logianalytics.com/hc/article_attachments/4404880682135/rpt_lsn7_title.gif)
3. Select the four DBFields in the summary area of the crosstab, then in the Report Inspector, specify the Height property to **0.25**, Width to **0.59**, and Format to **#,###.#**.
   ...
   ![Edit DBField Properties](https://devnet.logianalytics.com/hc/article_attachments/4404885682327/rpt_lsn7_fmtcrstb.gif)
...
1. Double-click the label **Quantity** in the chart, then in Format Label dialog box, switch to the **Font** tab, set Font Color to **808080**. Select **OK** to accept the settings.
   ...
   ![Format Chart Label](https://devnet.logianalytics.com/hc/article_attachments/4404880682519/rpt_lsn7_fmtlbl.gif)
...
1. Select the **Product ID** name label in the BPH panel and its corresponding field in the GH panel by holding the Ctrl key on the keyboard, then select **Format** > **Right**![Right Align button](https://devnet.logianalytics.com/hc/article_attachments/4404880682775/btn_alignright.gif) to make them aligned the same as other labels and fields in the banded object.
...
3. Select the **Unit Price** DBField and then specify its Format property to **$#,###.00** in the Report Inspector.
   ...
   ![Format Banded Object](https://devnet.logianalytics.com/hc/article_attachments/4404880683287/rpt_lsn7_fmtbnd.gif)

After doing the adjustments, Designer displays the report like follows in design view:

![Report in Design View](https://devnet.logianalytics.com/hc/article_attachments/4404885682583/rpt_lsn7_dsgn.gif)
...
5. Select the **View** tab. Designer displays the report as follows, depending on what formatting you have done to the components.
   ...
   ![Preview Report](https://devnet.logianalytics.com/hc/article_attachments/4404880683543/rpt_lsn7_rpt.gif)

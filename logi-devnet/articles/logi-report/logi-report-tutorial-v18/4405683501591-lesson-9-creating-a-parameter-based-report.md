...
3. In the **Data** panel, select **<New Parameter...>** in the Parameters node.
   ...
   ![New Parameter](https://devnet.logianalytics.com/hc/article_attachments/4420453732247/rpt_lsn9_prm.gif)
4. In the New Parameter dialog box, type **pStartDate** in the **Name** text box, keep the default **Value Setting**, select **Date** from the **Value Type** drop-down list, select **Add**![Add button](https://devnet.logianalytics.com/hc/article_attachments/4420453732503/btn_add.gif) to add a value row in the **Value List** box and type **May 1, 2016** as the prompting value, and then type **Please input start date:** as the prompting text. Select **OK** to create the parameter.

   ![Define the Parameter](https://devnet.logianalytics.com/hc/article_attachments/4420461790231/rpt_lsn9_crtprm.gif)
...
1. In the **Data** panel, select **Dataset Filter**![Dataset Filter button](https://devnet.logianalytics.com/hc/article_attachments/4420461790487/btn_fltr.gif) on the toolbar.
2. In the Dataset Filter dialog box, select **Add Condition** to add a filter line, select **ORDER DATE** from the field drop-down list, set **>=** as the operator, then select the **ellipsis**![Ellipsis button](https://devnet.logianalytics.com/hc/article_attachments/4420461790743/btn_chsr.gif).
3. In the Expressions dialog box, double-click the **pStartDate** parameter to use it as value of the filter condition.
   ...
   ![Filter Dataset](https://devnet.logianalytics.com/hc/article_attachments/4420453732759/rpt_lsn9_prmfltr.gif)
...
5. Specify condition of the newly added filter as follows:
   ...
   ![Dataset Filter Conditions](https://devnet.logianalytics.com/hc/article_attachments/4420447409559/rpt_lsn9_fltr.gif)
6. Select **OK** to apply the settings.

You can also use parameters to filter queries to limit their data. However, in this task, we cannot set the filter on the report's query. If we do that, all datasets based on the query will be affected, meaning, Designer also applies the filter to the report that we created in Lesson 1.
...
1. Save the report and select the **View** tab to preview the report.
   ...
   ![Enter Parameter Values](https://devnet.logianalytics.com/hc/article_attachments/4420447409815/rpt_lsn9_value.gif)
...
3. Select **Default** to generate the report output based on the default values for the parameters. Designer displays the report as follows, which shows information for orders from May 1, 2016 to June 1, 2016 only.
   ...
   ![Preview Report](https://devnet.logianalytics.com/hc/article_attachments/4420447410071/rpt_lsn9_rpt.gif)

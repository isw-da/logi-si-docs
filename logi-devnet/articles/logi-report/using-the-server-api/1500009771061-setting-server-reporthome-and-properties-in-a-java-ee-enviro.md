...
This implementation extends DefaultServerEnv. It enables finding the reporthome not only from the <context-param></context-param> or <env-entry></env-entry> tags, but also from an external file <user.home>/.jreportrc. However, this implementation has three main limitations as shown below:

* You must specify this implementation with the <env-entry></env-entry> tags in the target **web.xml** in makewar.xml or in ejb-jar.xml as follows:
...
2. Since different Logi Report instances cannot access the same Logi Report Server system database at the same time, you will need to create a dbconfig.xml file to store the connection information, and put it in jrenv.jar in `workspace\bin` (you can find the jrenv.jar file after extracting jreport.war) for each Logi Report Server WAR/EAR.
   ...
   For more information about modifying dbconfig.xml, see [Configuring the Server Database](https://devnet.logianalytics.com/hc/en-us/articles/1500009771521-Configuring-the-Server-Database).

...
url: https://devnet.logianalytics.com/hc/en-us/articles/5735555374103-Managing-the-Data-Retrieval-of-Queries
updated_at: 2022-11-02T08:17:44Z
---

# Managing the Data Retrieval of Queries

[![Back](https://devnet.logianalytics.com/hc/article_attachments/9898402961815/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/5735547165591-Applying-the-Filter-of-a-Query-to-Multiple-Queries)  [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/9898402971543/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/5735586187287-Working-with-Cached-Query-Results)

# Managing the Data Retrieval of Queries

The Data Manager in Designer enables you to control the data retrieval of your queries (and also [imported SQLs](https://devnet.logianalytics.com/hc/en-us/articles/5735507746711-Using-Imported-SQLs), [stored procedures](https://devnet.logianalytics.com/hc/en-us/articles/5735521490199-Using-Stored-Procedures), and [user-defined data sources](https://devnet.logianalytics.com/hc/en-us/articles/5735521621399-Using-User-Defined-Data-Sources) that function similarly as queries), including the number of rows to display and the duration allowed for retrieval. It can also keep access information from previous runs of a query. This topic describes how you can use the Data Manager to limit the run time and number of records of queries, and view the running histories of queries.

This topic contains the following sections:

* [Limiting the Query Run Time and Number of Records](#Limit)
* [Viewing a Query's Running History](#View)
  + [Viewing the Designer-Side Running History of a Query](#Designer)
  + [Viewing the Server-Side Running History of a Query](#Server)
...
1. Navigate to **File** > **Open** to open the page report **Payroll Report.cls** in the catalog **SampleReports.cat** saved in `<designer_install_root>\Demo\Reports\SampleReports`.
2. Adjust the **Maximum Rows** and **Maximum Duration** properties of the query **Payroll** the report uses to suitable levels. You can adjust the values using either of the following methods:
...
If your queries use data from a relational database, you can further customize where to implement the two properties: on Logi Report side or on the database side by configuring the file **JdbcDriversConfig.properties**, which is available in both Designer and Server in its `<install_root>\bin` directory. For example:
...
* Select the Refetch Data button ![Refetch Data button](https://devnet.logianalytics.com/hc/article_attachments/9898405299351/btn_refetchdt.gif) on the toolbar of the View tab to run the report again if you find that the specified properties do not take effect.
...
1. Open the file **report.ini** in `<designer_install_root>\bin` and add the command line **`alwaysShowDMRecord=true`** as follows. Here, it is assumed that you have installed Designer to `C:\LogiReport\Designer`. The escape character backslash following "=" and before each special character such as ":" and "\" are required.
...
5. In the Catalog Manager or the Data panel, right-click the query the report uses and select **Data Manager** from the shortcut menu. Designer displays the [Data Manager dialog box](https://devnet.logianalytics.com/hc/en-us/articles/5735513614999-Data-Manager-Dialog-Box). The dialog box shows the running information of the query, including when the report is run - date and time, the running duration, the number of rows displayed, and the report that uses the query. You can sort the items by Date, Time, Number of Rows, Duration, or Report, by selecting the column title.

   ![Data Manager dialog box](https://devnet.logianalytics.com/hc/article_attachments/9898475251351/dtmngr.gif)
...
2. Navigate to **Home** > **Catalog Manager**.
...
7. Use the query Payroll to [create another page report](https://devnet.logianalytics.com/hc/en-us/articles/5735569787671-Creating-Reports#Page) report1.cls.
8. Preview report1.cls. Designer appends the running information as a record in the Data Manager dialog box.
...
2. [Publish a page report](https://devnet.logianalytics.com/hc/en-us/articles/5735576653207-Publishing-and-Downloading-Resources#Remotely) that has its query properties Maximum Rows and/or Maximum Duration set together with its catalog to Server.
3. Run the report on Server. Server saves the query running information, but does not have a GUI to display it. In order to view the information, you need to use the Data Manager in Designer.
...
5. In Designer, use the Data Manager to view the query's Server-side running history.

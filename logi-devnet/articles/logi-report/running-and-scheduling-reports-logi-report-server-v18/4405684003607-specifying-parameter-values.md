...
url: https://devnet.logianalytics.com/hc/en-us/articles/4405684003607-Specifying-Parameter-Values
...
* In the parameter value combo box, select the down arrow ![More button](https://devnet.logianalytics.com/hc/article_attachments/4420453419415/btn_mvdown1.gif) and then select a value from the drop-down list.
  ...
  You can make use of the Sort button ![Sort button](https://devnet.logianalytics.com/hc/article_attachments/4420447138711/btn_sort2.gif) to display the values in the ascending or descending order for easy look-up, and search for the required value conveniently in the search box. To cancel the search, clear the input text or select the delete button ![Close button](https://devnet.logianalytics.com/hc/article_attachments/4420453394455/btn_delete.gif) in the search box.
...
* Select the down arrow ![More button](https://devnet.logianalytics.com/hc/article_attachments/4420453419415/btn_mvdown1.gif) in the parameter value combo box to [specify multiple values](#MultiValue) if the parameter enables multiple values.
* Select or clear the checkbox to specify a Yes or No value.
* Select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4420461463447/btn_clndr.gif) to [specify a date and time value](#DateTime).

![Note icon](https://devnet.logianalytics.com/hc/article_attachments/4420447034135/noteicon.jpg)You should not use blank as the thousands separator in Number-typed parameter values under French locale, otherwise Server does not correctly recognize your input because of a JVM bug. For more information, see <http://bugs.sun.com/view_bug.do;jsessionid=c8cdaf911b20fffffffffd9fc6340b30d670?bug_id=4510618>.
...
1. Select the down arrow ![More button](https://devnet.logianalytics.com/hc/article_attachments/4420453419415/btn_mvdown1.gif) in the value combo box of the parameter. Server displays the [Enter Values](https://devnet.logianalytics.com/hc/en-us/articles/4405690548759-Enter-Values-Dialog-Box-Properties) dialog box.
...
3. Select the required values in the **Available Values** box and select the button ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4420447121559/btn_add.gif) to add them to the **Selected Values** box; to add all the listed values, select the button ![Add All button](https://devnet.logianalytics.com/hc/article_attachments/4420447138967/btn_addall.gif).

   To ensure performance, by default, Server lists at most 500 values in the **Available Values** box. When the listed values are just a part of the values of the parameter, after you scroll down to the bottom of the list, Server automatically loads more values (500 at most) into the list. You can make use of the Sort button ![Sort button](https://devnet.logianalytics.com/hc/article_attachments/4420447138711/btn_sort2.gif) to display the values in the ascending or descending order for easy look-up, and the Search button ![Search button](https://devnet.logianalytics.com/hc/article_attachments/4420453392407/btn_srch.gif) to search for the required values conveniently. To cancel the search and close the search box, select the delete button ![Close button](https://devnet.logianalytics.com/hc/article_attachments/4420453394455/btn_delete.gif) in it.

   For values you do not want, select them in the **Selected Values** box and select the button ![Remove button](https://devnet.logianalytics.com/hc/article_attachments/4420461527319/btn_rmv.gif) to remove them; to remove all added values, select the button ![Remove All button](https://devnet.logianalytics.com/hc/article_attachments/4420461549975/btn_rmvall.gif).
4. When the parameter's **Allow Type-in of Value** property is true, the **Enter Values** text box is available. You can type a value manually and select the button ![Add button](https://devnet.logianalytics.com/hc/article_attachments/4420447121559/btn_add.gif) next to add it to the **Selected Values** box. When the parameter is bound with a column, but the display column is different from the bound column, make sure the value you type is that of the bound column. If the parameter is of the Date, DateTime, or Time type, you can also select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4420461463447/btn_clndr.gif) to specify a date and time value using the calendar.

   ![Enter Values dialog - allow type in](https://devnet.logianalytics.com/hc/article_attachments/4420461550231/wkrpt_prmtr_entervlu.gif)
...
1. Select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4420461463447/btn_clndr.gif) for the parameter. Server displays the following dialog box.

   ![Calendar dialog](https://devnet.logianalytics.com/hc/article_attachments/4420447139223/wkrpt_prmtr_clndr.gif)
...
4. If the parameter is of DateTime type, the time setting is available. From the **Display Time Format** drop-down list, select in which format to display the time, **12 hour** or **24 hour**. However, each time you access the calendar, the default format is always 12 hour.
   ...
   ![Calendar dialog](https://devnet.logianalytics.com/hc/article_attachments/4420461550487/wkrpt_prmtr_clndrtm.gif)
...
1. Select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4420461463447/btn_clndr.gif) for the parameter. Server displays the following dialog box.
   ...
   ![Calendar dialog](https://devnet.logianalytics.com/hc/article_attachments/4420447139479/wkrpt_prmtr_exprsn.gif)
...
4. You can make use of Logi Report's built-in Date/Time functions in the expression. First put the cursor where you want to insert the function in the **Expression** text box, select the ellipsis button ![Choose button](https://devnet.logianalytics.com/hc/article_attachments/4420453393175/btn_chsr2.gif) next to the **Template** text box to access the function list, and then select a function to insert it into the **Expression** text box.
...
* If the edited expression has errors, Server does not create a value, and displays the expression itself and highlights it in red in the value text box. Correct the expression, or select the **Calendar** icon ![Calendar icon](https://devnet.logianalytics.com/hc/article_attachments/4420461463447/btn_clndr.gif) to specify a value using the calendar.
...
Logi Report Server enables you to pre-customize the default parameter values for a report before running it. To use this feature, make sure you clear neither [Enable Setting Default Parameter Values For](https://devnet.logianalytics.com/hc/en-us/articles/4405683590167-Configuring-Server-Profile#DefaultParam) nor [Enable Hiding Initial Parameter Dialog For](https://devnet.logianalytics.com/hc/en-us/articles/4405683590167-Configuring-Server-Profile#HideParam) the corresponding report type in the server profile.

1. Browse to the target report, then do one of the following:
   * Put the mouse pointer over the report row and select the **Parameter Settings** button ![Parameter Settings button](https://devnet.logianalytics.com/hc/article_attachments/4420447139735/btn_srv_prmset.gif) on the floating toolbar.
   ...
   * Select the report row, then select **Tools** > **Parameter Settings** on the task bar.
   ...
   Server displays the [Parameter Settings](https://devnet.logianalytics.com/hc/en-us/articles/4405683757079-Parameter-Settings-Dialog-Box-Properties) dialog box.

   ![Parameter Settings dialog](https://devnet.logianalytics.com/hc/article_attachments/4420461550743/prmset.gif)
...
![Note icon](https://devnet.logianalytics.com/hc/article_attachments/4420447034135/noteicon.jpg)If you just edit the parameter values in the dialog box and select **OK**, the new values are meaningless. Only when you select **Save as default** and select **OK**, can Server save the values as default values to the report.
...
To switch on the function of saving parameter values, you need to configure the server profile: select **Yes** for the option [Enable Saving Parameter Values](https://devnet.logianalytics.com/hc/en-us/articles/4405683590167-Configuring-Server-Profile#SaveParam), select a way to do the saving: Manually or Automatically, and then specify a maximum number to limit the saved value groups via the option [Maximum Number of Auto Complete Parameters List](https://devnet.logianalytics.com/hc/en-us/articles/4405683590167-Configuring-Server-Profile#MaxNo) for each user-report pair.

**Manually saving parameter values**

When you have chosen to manually save parameter values, Server displays the Use Saved Values button **![Use Saved Values button](https://devnet.logianalytics.com/hc/article_attachments/4420447139991/btn_srv_ussvvlu.gif)** at the upper right corner of the parameter page. By selecting this button, you will get the following:

* **Value list**  
   The value list contains all the parameter value groups you have saved for the report. Select a group to apply all the values in the group to the corresponding parameters. You can also remove any of the unwanted value group by selecting it from the list and then selecting the delete button ![Delete button](https://devnet.logianalytics.com/hc/article_attachments/4420461456151/btn_dltobj.gif) next to the value list.
* **![Save Values button](https://devnet.logianalytics.com/hc/article_attachments/4420461454871/btn_save.gif) Save Values**  
   Saves the current displayed parameter values as a group for reuse afterwards. To do this, type a name for the group in the text box which shows "Save current values into list" and then select the **Save Values** button ![Save Values button](https://devnet.logianalytics.com/hc/article_attachments/4420461454871/btn_save.gif). Server adds the saved group into the value list.

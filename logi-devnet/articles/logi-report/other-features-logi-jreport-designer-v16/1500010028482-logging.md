...
By default, all log categories use one appender, JReportRollingFile. However, you can also define different output appenders or files for different log categories in the LogConfig.properties file, which is located in `<install_root>\bin`. For example, to add another log appender, JReportConsole, you can edit LogConfig.properties as follows:

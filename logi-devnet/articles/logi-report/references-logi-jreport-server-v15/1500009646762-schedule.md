...
url: https://devnet.logianalytics.com/hc/en-us/articles/1500009646762-Schedule
updated_at: 2021-07-24T20:54:44Z
---

# Schedule

[![Back](https://devnet.logianalytics.com/hc/article_attachments/4404920354071/back.png)Previous Topic](https://devnet.logianalytics.com/hc/en-us/articles/1500009671081-Result-Properties)   [Next Topic![Next](https://devnet.logianalytics.com/hc/article_attachments/4404920354327/forward.png)](https://devnet.logianalytics.com/hc/en-us/articles/1500009646822-Select-Another-Catalog)

# Schedule

Logi JReport Server allows you to [schedule a report](https://devnet.logianalytics.com/hc/en-us/articles/1500009649902-Scheduling-Tasks-to-Publish-Reports) to run at a specific time and publish the report result to different formats.

To access the Schedule dialog, on the Logi JReport Console > Resources page, browse to the report you want to schedule, then do either of the following:

* Select the row the report is in, then on the task bar of the Resources page, select Run > Schedule.
...
This dialog contains the following tabs. Set the settings in each tab according to your requirements.
...
Before you can schedule a task, you first need to configure the settings in this tab. [See the tab](javascript:%20void(null)).
...
* **Export to One File**If checked, the selected report tabs will be exported to one result file.
...
* **Connection Properties**  
   Shows the [information](https://devnet.logianalytics.com/hc/en-us/articles/1500009646442-Edit-Dynamic-Connection) of the selected dynamic connection, which is read only.
...
* **Select Another Catalog**  
   Specifies another catalog for the report in the [Select Another Catalog](https://devnet.logianalytics.com/hc/en-us/articles/1500009646822-Select-Another-Catalog) dialog.
* **Report Version**  
   Specifies the report version used for the schedule task. The default version is always the latest version.
* **Catalog Version**  
   Specifies the catalog version used for the schedule task. The default version is always the latest version.
* **Priority**  
...
* **Use User-Defined Default On-screen Filter Values**  
   Applies the default on-screen filter values specified to the report for the user. The option is available when [Enable Setting Default On-screen Filter Values For](https://devnet.logianalytics.com/hc/en-us/articles/1500009644582-Configuring-Server-Profile#OnScreenFilter) the corresponding report type is selected in the Profile > Customize Server Preferences > Advanced tab.

[![](https://devnet.logianalytics.com/hc/article_attachments/4404920354583/totop.png)Back to top](#top "Click to go to the top")

## Parameter

This tab allows you to specify the parameter values if any to run the report. [See the tab](javascript:%20void(null)).

![](https://devnet.logianalytics.com/hc/article_attachments/4404920561047/sch_prm.gif)

**Enter Parameters**

Displays parameters of the report with your last-time saved default values, which could be the values saved in this tab last time, or in the [Enter Parameter Values](https://devnet.logianalytics.com/hc/en-us/articles/1500009646482-Enter-Parameter-Values) dialog or [Parameter Settings](https://devnet.logianalytics.com/hc/en-us/articles/1500009646702-Parameter-Settings) dialog, or when [advanced running the report](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#Parameter), and for a web report, could also be the values saved in the [Parameters panel](https://devnet.logianalytics.com/hc/en-us/articles/1500009650342-Applying-Parameters#Panel). Edit the values according to your requirement. [You may specify parameter values in these ways](https://devnet.logianalytics.com/hc/en-us/articles/1500009649882-Specifying-Parameter-Values).
...
* [To Disk](#Disk)  
   Publishes the report result to the file system. This option is not available to [organization users](https://devnet.logianalytics.com/hc/en-us/articles/1500009675081-Multi-tenancy-Supported-via-Organizations).
* [To E-mail](#Mail)  
...
If you select User Task to publish your reports, you can implement a customized task with schedule properties. This option is not available to [organization users](https://devnet.logianalytics.com/hc/en-us/articles/1500009675081-Multi-tenancy-Supported-via-Organizations).
...
This tab is for you to specify settings for publishing the report result to the versioning system. [See the tab](javascript:%20void(null)).
...
* **Logi JReport Result**  
   Publishes the page report to a Logi JReport result file (RST file). RST files can be exported to HTML, PDF, Text, Excel, XML, RTF, Applet and Postscript formats via
  Logi JReport Server.
  + **Zip Result**   
     Specifies to compress the result to reduce the disk size and I/O; however, it uses more CPU resources.
  + **Run Linked Report**If the report is linked with other reports, you can check whether or not to generate the linked reports (not including the detail reports) in the RST file. If you are only interested in the primary report, leave this option unchecked. Generating linked reports at the same time, especially when the linked reports contain a large amount of data, will cause performance issue.
  + **Precision Level**  
...
* **Page Report Result**  
  ...
  + **Precision Level**  
     Specifies the precision level with which to publish the report result. Note that changing the default value may cause abnormalities in report layout.
* **Web Report Result**  
   Publishes the web report to a static web report result file (WST file). WST files can be exported to HTML, PDF, Text, Excel, XML, RTF, and Postscript formats via Logi JReport Server.
  + **Zip Result**   
     Specifies to compress the result to reduce the disk size and I/O; however, it uses more CPU resources.
  + **Run Linked Report**  
...
* **Built-in Version Folder**  
   Specifies to save the report result version to the built-in version folder. This option is not available to [organization users](https://devnet.logianalytics.com/hc/en-us/articles/1500009675081-Multi-tenancy-Supported-via-Organizations) when the report is from the Public Reports folder.
* **My Reports****Folder**  
...
* **Result Expires After**Specifies a certain day on which the report result will be automatically deleted.

**Set Permissions**

Available only when the Archive Location is specified to be Public Reports Folder. Select the link to set user permissions to the specified report tabs or web report in the [Set Permissions](https://devnet.logianalytics.com/hc/en-us/articles/1500009670941-Set-Permissions) dialog.

### To Disk

This tab is for you to specify settings for publishing the report result to the file system. [See the tab](javascript:%20void(null)).
...
* [Logi JReport Result](#RST)Publishes the page report to a Logi JReport result file (RST file).
* [Page Report Result](#RSD)   
   Publishes the page report to a page report result file (RSD file).
...
When publishing a page report, if you choose the Logi JReport Result and Page Report Result formats, you need only provide one file path because all selected report tabs will be output into one file. For other formats, if you leave the Export to One File option in the General tab unchecked, you need to specify a file path for each selected report tab.

### To E-Mail

This tab is for you to specify settings for publishing the report result to e-mail. [See the tab](javascript:%20void(null)).

![](https://devnet.logianalytics.com/hc/article_attachments/4404920561303/sch_pub_mal.gif)
...
* **Use Default SMTP Account**  
   Specifies whether to use the default SMTP account configured on the Logi JReport Administration > Configuration > [E-mail Server page](https://devnet.logianalytics.com/hc/en-us/articles/1500009669181-Configuring-the-E-Mail-Server) as the e-mail publish account.
...
* **E-mail Result in HTML E-mail Format**  
  ...
  + **Run Linked Report**If the report is linked with other reports, you can check whether or not to generate the linked reports (not including the detail reports) in the HTML result. If you are only interested in the primary report, leave this option unchecked. Generating linked reports at the same time, especially when the linked reports contain a large amount of data, will cause performance issue.
* **E-mail Result in Plain Text E-mail Format**  
   Sends the report result via e-mail to the specified address in plain [text](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#Text) format. The report result will be shown in plain text format in the mail body with no other information such as color, images and so on.
...
* **Attachment in HTML Format**  
   Sends the report result via e-mail to the specified address with an [HTML](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#HTML) file as attachment.
* **Attachment in PDF Format**  
   Sends the report result via e-mail to the specified address with a [PDF](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#PDF) file as attachment.

  **Note:** If the [Split PDF](https://devnet.logianalytics.com/hc/en-us/articles/1500009644522-Configuring-Export-Settings#SplitPDF) is enabled in the Configuration > Export > E-mail tab on the Logi JReport Administration page, the TOC and Sign options are disabled here.
* **Attachment in Excel Format**  
   Sends the report result via e-mail to the specified address with an [Excel](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#Excel) file as attachment.
* **Attachment in Text Format**  
   Sends the report result via e-mail to the specified address with a [Text](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#Text) file as attachment.
* **Attachment in RTF Format**  
   Sends the report result via e-mail to the specified address with a [RTF](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#RTF) file as attachment.
* **Attachment in XML Format**  
   Sends the report result via e-mail to the specified address with an [XML](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#XML) file as attachment.
* **Attachment in PostScript Format**  
   Sends the report result via e-mail to the specified address with a [PostScript](https://devnet.logianalytics.com/hc/en-us/articles/1500009670701-Advanced-Run#PS) file as attachment.
* **OK**  
...
This tab is for you to specify settings for publishing the report result to a printer. [See the tab](javascript:%20void(null)).

![](https://devnet.logianalytics.com/hc/article_attachments/4404926735639/sch_pub_prnt.gif)
...
This tab is for you to specify settings for publishing the report result to fax. [See the tab](javascript:%20void(null)).

![](https://devnet.logianalytics.com/hc/article_attachments/4404920561687/sch_pub_fax.gif)
...
Specifies the date on which the fax is to be sent. You can specify the date by selecting the calendar button ![](https://devnet.logianalytics.com/hc/article_attachments/4404920401431/btn_clndr.gif).
...
This tab is for you to specify settings for sending the report result to an FTP site. [See the tab](javascript:%20void(null)).

![](https://devnet.logianalytics.com/hc/article_attachments/4404920561943/sch_pub_ftp.gif)
...
The Time tab allows you to set settings for specifying the time for when the task is to be performed. [See the tab](javascript:%20void(null)).
...
Creates a new trigger. This option is not available to [organization users](https://devnet.logianalytics.com/hc/en-us/articles/1500009675081-Multi-tenancy-Supported-via-Organizations).
...
The Notification tab allows you to notify someone by e-mail when a task finishes running, regardless of whether it is successful or unsuccessful. [See the tab](javascript:%20void(null)).
...
This tab is available only when you have [enabled the task-level timeout mechanism](https://devnet.logianalytics.com/hc/en-us/articles/1500009648962-Task-level-Timeout-for-Advanced-Run-and-Schedule-Tasks). In this tab, you can specify a time duration for a task, and ask Logi JReport Server to cancel the task or to notify you or someone else of the task status via e-mail if the task has not yet finished running when the task duration is up. [See the tab](javascript:%20void(null)).

![](https://devnet.logianalytics.com/hc/article_attachments/4404926736023/sch_drtn.gif)

...
category: "Izenda"
url: https://devnet.logianalytics.com/hc/en-us/articles/12528207540631-ReactJS-to-Manage-a-Scalar-Value
updated_at: 2023-02-23T15:09:43Z
---
...
In this sample we will write a simplistic web page utilizing ReactJS to manage a server scalar value setting.

* We will test with the [GET report/reportMode](https://devnet.logianalytics.com/hc/en-us/articles/12528279020567-Report-Designer-Details-APIs#_bm10) and [POST report/reportMode/{value}](https://devnet.logianalytics.com/hc/en-us/articles/12528279020567-Report-Designer-Details-APIs#_bm13) APIs.
* We will use ReactJS to create the UI.

## Prerequisites

1. This sample is a continuation of [REST API Test](https://devnet.logianalytics.com/hc/en-us/articles/12528223416343-REST-API-Test).
2. This sample re-uses the folder structure created in [REST API Test](https://devnet.logianalytics.com/hc/en-us/articles/12528223416343-REST-API-Test).
...
1. In “ui\_react\_examples” folder, create a blank text file and name it “ReactJS to manage a scalar value.html”.
...
For production web site, we will replace the local libraries with hosted, minified and compressed library links.

## ReactJS basics

A ReactJS component maintains its data in `this.state`, initialize the data in `getInitialState` method, load the data from server in `componentDidMount` method, and creates its UI elements in the `render` method. The `render` method is called each time there is a change in `this.state`.
...
Just by implementing these 6 methods then we will have a working ReactJS component. In this specific case, we only need to implement fetchData and pushData, the other boilerplate methods are already working without any change needed.
...
From the API documentation [GET report/reportMode](https://devnet.logianalytics.com/hc/en-us/articles/12528279020567-Report-Designer-Details-APIs#_bm11), the `//TODO` in fetchData would be replaced by this ajax call:
...
From the API documentation [POST report/reportMode/{value}](https://devnet.logianalytics.com/hc/en-us/articles/12528279020567-Report-Designer-Details-APIs#_bm12), the `//TODO` in pushData would be replaced by this ajax call:
...
Now we can enter the desired value 0 or 1 and click Save to update the server setting. Any other value would receive a false response and an notice line in JavaScript Console.
...
* added event handlers to get data, post data and update the state data according to user input.

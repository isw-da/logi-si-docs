...
category: "Logi Info"
url: https://devnet.logianalytics.com/hc/en-us/articles/4406635302551-Using-Excel-and-CSV-Data
...
One of the features Logi DataHub v3 provides is the ability to import and use data from Microsoft Excel spreadsheet (.xlsx and .xls) and comma-separate values (.csv) files. This topic discusses preparing and importing this kind of data for use with Dataviews.

* [Preparing the Data](#Preparin)
* [Uploading Data Files](#Upload)

For basic information about creating Dataviews, see *[Dataview Authoring](https://devnet.logianalytics.com/hc/en-us/articles/4406817347863-Dataview-Authoring)*.

## Preparing the Data

Before using Excel data with a Dataview, it's important to understand it. Many Excel files are basically formatted reports. The raw data may have been annotated, aggregated, sorted, formatted, and grouped. Unfortunately, it can be difficult to extract meaningful data from this kind of file. In these cases, the data in the Excel file will need to be "massaged" before it can be used.

Logi DataHub v3 consumes only raw data. In order for an Excel file to be imported cleanly into Logi DataHub v3, it must follow these three rules:
...
* Each worksheet in an Excel file is considered a different data object in Logi DataHub v3.

The first two rules also apply to comma-separated values (CSV) data files for use with Logi DataHub v3.

### Cleaning Up the Data

Excel data to be loaded into Logi DataHub v3 should be "de-normalized", raw data. The worksheet should be edited to:
...
Empty cells and cells with Null values are considered data once imported into Logi DataHub v3. For example, in the incorrect example worksheet shown earlier, all of the cells that are subordinate to the grouped data are empty. These cells must be filled to correlate the data values to the proper group.
...
Every column must have header text. This text will be used to identify the column data in Logi DataHub v3. After formatting your Excel worksheet, make sure that each column is properly identified in the first row of the worksheet.

### Make the Data Homogeneous

Within each column, all data should be of one data type. Logi DataHub v3 examines the data during import and sets the data type
(dimension, measure, date, identity) according to column contents, so it's important that all of the data be of the same type.

Excel worksheet column formatting can be tricky here. For example, in the worksheet shown earlier, the *Order ID* column appears to be numeric and would therefore be assumed to be a "measure" in Logi DataHub v3. However, the *Order ID* is actually text data (as you can tell by the small, green triangle in the upper left corner of each *Order ID* cell) and would actually be imported into Logi DataHub v3 incorrectly as a "dimension".

### Importing Multiple Worksheets

When an Excel file is identified in a Dataview as a data source,
...
![](https://devnet.logianalytics.com/hc/article_attachments/5160418719383/usingexcel_05.png)
...
When imported into Logi DataHub v3, they will be presented as two separate data objects, shown highlighted in yellow above.

## Uploading Data Files

In the Dataview Authoring tool's **Create a New Dataview** page, there are two
...
![](https://devnet.logianalytics.com/hc/article_attachments/5160453729303/noteicon.jpg) If you're having trouble importing a CSV file and you suspect that it may be due to special characters in the data, try opening the file in Excel first, saving it as an Excel file, and then importing the Excel file.

![](https://devnet.logianalytics.com/hc/article_attachments/5160434343575/usingexcel_10.png)

A Dataview allows you to blend data from

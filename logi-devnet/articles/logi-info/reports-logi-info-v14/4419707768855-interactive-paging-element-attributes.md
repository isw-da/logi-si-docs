...
category: "Logi Info"
url: https://devnet.logianalytics.com/hc/en-us/articles/4419707768855-Interactive-Paging-Element-Attributes
...
| --- | --- |
| Caption Type | (Required) Specifies whether the navigation links will be Text, Images, or Scrollbar  New for 14.1. |
...
| New for 14.2 Lazy Loading | Specifies whether the scrollbar utilizes lazy loading. The default value is *False*, all data loads at one time. When set to *True*, loading begins when the scrollbar reaches the bottom of the page. For this to apply, the **Caption Type** attribute must be set to *Scrollbar*.  Depending on your data results, lazy loading may expand your table width while scrolling. Once you reach the bottom of your results the table width is final. |
| New for 14.2 Lazy Loading Multiple | Specifies the amount of data that fetches on the next load. The default value is empty or *1*, the next load fetches page count row data. If set to 3, the next load fetches 3x the page count row data. For this to apply, the **Caption Type** attribute must be set to *Scrollbar* and **Lazy Loading** must be set to *True*.  Depending on your data results, lazy loading may expand your table width while scrolling. Once you reach the bottom of your results the table width is final. |

...
url: https://devnet.logianalytics.com/hc/en-us/articles/4405850951959-Elasticsearch-Source-Document-Storage-Configurations
updated_at: 2021-09-21T01:27:25Z
---
...
Elasticsearch data stores generally store the original JSON source documents passed when Elasticsearch performs its document indexing in the `_source` field in the index. However, some organizations disable the the `_source` field to save storage and thus do not store the original JSON source documents.

Composer Elasticsearch 6 and 7 connectors support Elasticsearch data stores with any of the following source document configurations:

...
url: https://devnet.logianalytics.com/hc/en-us/articles/4402955595927-Remove-Nodes-from-a-High-Availability-Environment
updated_at: 2021-08-07T17:31:12Z
---
...
To remove Composer nodes (or instances) from a high availability environment:
...
2. If the instance you are removing will be run standalone, edit the `zoomdata.properties``and query-engine.properties` files to point at a local PostgreSQL data store for that instance. Ensure that the JDBC settings point to the local PostgreSQL and that the user name and password used to access the local PostgreSQL data store are correct in these files. After changing the settings, restart all the Composer microservices for the instance. See [*Restart Composer Microservices*](https://devnet.logianalytics.com/hc/en-us/articles/4402963006871-Restart-Composer-Microservices).

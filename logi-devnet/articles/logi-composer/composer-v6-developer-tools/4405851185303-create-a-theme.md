...
url: https://devnet.logianalytics.com/hc/en-us/articles/4405851185303-Create-a-Theme
updated_at: 2024-03-24T15:30:28Z
---
...
1. Set up the JSON file that describes your theme. Download and use the JSON files provided with the supplied **modern** and **dark** themes to get started or create your own.
   ...
   * To use the JSON files for either the **modern** or **dark** theme as a template for your own, retrieve the theme JSON file using the `/api/customization/themes/name/<name>` API endpoint in a GET request. The following request obtains the JSON for the **modern** theme.
     ...
     ```
     curl -X GET "http://<ip-address>:<port>/composer/api/customization/themes/name/modern" -H "accept: application/vnd.composer.v2+json"
     ```
   ...
   * You can create a JSON file that describes your theme. A sample is provided in [*Sample Themes JSON File*](https://devnet.logianalytics.com/hc/en-us/articles/4405851186327-Sample-Themes-JSON-File).
2. Use the `/api/customization/themes/`[API endpoint](https://devnet.logianalytics.com/hc/en-us/articles/4405851184407-Themes-API-Endpoint) in a POST request to create the theme. Use the text of your JSON file as the body (`"content": "<string>"`) of the request. Be sure to remove the `"system": true` and `"id": "<string>"` properties from the JSON file before you use it to create a theme. Composer automatically generates an ID for the theme and sets the value of the `"system"` property when the API request to create the theme is processed.
   ...
   ```
   curl -X POST "<ip-address>:<port>/composer/api/customization/themes" -H "accept: application/vnd.composer.v2+json" -H "Content-Type: application/json" -d "{ \"masterThemeId\": \"composer\", \"name\": \"<theme-name>\", \"content\": { <JSONcontent> } }
   ```
   ...
   After the theme is successfully created, it shows up in the list of available themes when one is generated (see [*List Themes*](https://devnet.logianalytics.com/hc/en-us/articles/4405859479831-List-Themes)) and it can be updated and patched (see [*Update a Theme*](https://devnet.logianalytics.com/hc/en-us/articles/4405851189399-Update-a-Theme) and [*Patch a Theme*](https://devnet.logianalytics.com/hc/en-us/articles/4405851187351-Patch-a-Theme)). It can also be deleted (see [*Delete a Theme*](https://devnet.logianalytics.com/hc/en-us/articles/4405859478551-Delete-a-Theme)). However, it will not be used by the Composer UI until it has been activated (see [*Activate a Theme*](https://devnet.logianalytics.com/hc/en-us/articles/4405859476887-Activate-a-Theme)).

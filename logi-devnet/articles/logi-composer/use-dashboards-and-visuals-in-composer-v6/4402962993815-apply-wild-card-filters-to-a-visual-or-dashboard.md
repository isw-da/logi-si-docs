...
url: https://devnet.logianalytics.com/hc/en-us/articles/4402962993815-Apply-Wild-Card-Filters-to-a-Visual-or-Dashboard
...
See [*Wild Card Case-Insensitive Filters*](https://devnet.logianalytics.com/hc/en-us/articles/4402955464471-Wild-Card-Case-Insensitive-Filters) and [*Wild Card Case-Sensitive Filters*](https://devnet.logianalytics.com/hc/en-us/articles/4402955465111-Wild-Card-Case-Sensitive-Filters) to determine whether case-insensitive or case-sensitive wild cards are supported.
...
3. On the Row tab, select the filter attribute you want to use from the list of available attributes. A second page with three tabs appears in the Filters sidebar.
   ...
   * The Value tab allows you to select a filter value for the attribute. See [*Apply Row-Level Filters*](https://devnet.logianalytics.com/hc/en-us/articles/4402955576599-Apply-Row-Level-Filters) for instructions on using a regular row-level filter.
   ...
   * The Keyset tab allows you to select a keyset filter for the attribute. See [*Use Keysets*](https://devnet.logianalytics.com/hc/en-us/articles/4402955615511-Use-Keysets) for instructions on using keysets in a filter.
...
8. If you want to specify another wild card value, select ![](https://devnet.logianalytics.com/hc/article_attachments/4404959479319/add.png) next to the **Values** title on the Wildcard tab. Then repeat Steps 5-7 above for the new wild card value. Multiple wild card values are treated as OR operations. A record can meet the filter criteria specified by any of the wildcard values to be selected for filter processing.
9. Select ![](https://devnet.logianalytics.com/hc/article_attachments/4404952853015/apply1-btn_48x27.png). If you create the filter at the dashboard level, it is applied to all the visuals in the dashboard. Otherwise, it is applied only to the selected (active) visual.
10. Optionally, repeat these steps to apply additional filters to the visual or dashboard.

See [*Remove a Filter from a Visual or Dashboard*](https://devnet.logianalytics.com/hc/en-us/articles/4402955579159-Remove-a-Filter-from-a-Visual-or-Dashboard) for information on removing a wild card filter from a visual or dashboard.

...
Composer does not allow you to remove a field from a source definition if any objects rely on this field. For example, if your native field `field_1` is used in a derived field expression such as `year_to_time(field_1)`, you can not remove it from the source.

The table below is a list of objects where a field can be used.

| Composer Object Type Referencing the Field | can not Delete from Source Configuration | Can Delete from Source Configuration |
| --- | --- | --- |

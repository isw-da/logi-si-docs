---
title: "Customize the Footer"
id: 34932678543757
section: "Composer 25 Developer Tools"
product: "Logi Composer v25"
url: https://logi-composer-v25.insightsoftware.com/hc/en-us/articles/34932678543757-Customize-the-Footer
updated_at: 2026-08-24T20:33:44Z
---

# Customize the Footer

# Customize the Footer

You can add custom footer for each page of the Composer UI. You can also specify the footer height.

The footer appears below the normal Composer UI page.

**To customize the footer:**

1. Log in as a system [admin](https://logi-composer-v25.insightsoftware.com/hc/en-us/articles/34932621360909-Supplied-Users-and-User-Groups#The2) or a member of the Supervisors group.

   **Note:** 
   The default **supervisor** user is no longer installed; add users to the **Supervisors** group instead.
2. Select **Customize UI** from the menu (![](https://logi-composer-v25.insightsoftware.com/hc/article_attachments/48374883779341)). The Customize UI page appears. The following screen shows all the custom footer settings on the page.

   ![](https://logi-composer-v25.insightsoftware.com/hc/article_attachments/48374002146701)
3. Under Custom Footer, enable the custom footer by sliding the **Custom Footer** slider to the right (on). You can always disable the custom footer by sliding the **Custom Footer** slider to the left (off). By default, the custom footer is off.
4. In the box under the Custom Footer slider, specify HTML structures for your custom footer. You can either add inline styles or include the classes corresponding to your [custom CSS file](https://logi-composer-v25.insightsoftware.com/hc/en-us/articles/34932670712205-Customize-the-Application#Upload).
5. In the **Custom Height in Pixels** box, specify the height (in pixels) of the footer. You can use the up and down arrows in the box to increment and decrement this value or you can type a value in the box.
6. Select **Save** to save and apply your changes.

   The footer appears below the normal Composer UI page.
